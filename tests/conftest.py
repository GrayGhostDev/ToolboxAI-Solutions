...
from tests.backend.test_logger import TEST_LOG_DIR
...
def sample_plugin_data():
    ...
    return {"studio_id": "test-studio-123", "port": 64989, "version": "1.0.0"}
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if hasattr(session, "cleanup_manager"):
        ...
        logger.info(
            f"Test cleanup completed: {stats['files_removed']} files removed, "
...
async def isolated_redis():
    ...
    client = await redis_async.from_url(TEST_REDIS_URL, decode_responses=True, max_connections=10)
...
async def shared_redis_pool():
    ...
    try:
        pool = await redis.create_pool(
            "redis://localhost:6379", minsize=5, maxsize=10, encoding="utf-8"
        )
        yield pool
    except Exception as e:
...
def fix_all_tests():
    ...
    patches = [
        ...
        patch(
            ...
            side_effect=mock_validate_origin,
        ),
    ]
