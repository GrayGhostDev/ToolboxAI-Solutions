...
from core.agents.supabase.supabase_migration_agent import SupabaseMigrationAgent, MigrationPlan
from core.agents.supabase.tools.schema_analyzer import SchemaAnalyzerTool, TableInfo
from core.agents.supabase.tools.rls_policy_generator import RLSPolicyGeneratorTool, RLSPolicy, PolicyType
from core.agents.supabase.tools.data_migration import DataMigrationTool, MigrationBatch, MigrationResult, MigrationStatus
        ...
        'tables': [
            {
                ...
                'foreign_keys': [],
                'indexes': [{'name': 'idx_organizations_domain', 'column_names': ['domain'], 'unique': True}],
                'unique_constraints': [{'name': 'uq_organizations_domain', 'column_names': ['domain']}],
                'check_constraints': [],
                'row_count': 100,
                'size_bytes': 32768
            },
        ...
        rollback_procedures=[
            {
                ...
                'commands': ['DROP TABLE IF EXISTS users CASCADE;', 'DROP TABLE IF EXISTS lessons CASCADE;']
            },
            {
                'step': 'rls',
                'procedure': 'rollback_rls',
                'commands': ['DROP POLICY IF EXISTS users_select_own;', 'DROP POLICY IF EXISTS lessons_public_select;']
            }
        ],
...
class SupabaseMigrationTestHelper:
    ...
    def calculate_migration_metrics(plan: MigrationPlan) -> Dict[str, Any]:
        ...
        total_rows = sum(
            migration.get('estimated_rows', 0)
