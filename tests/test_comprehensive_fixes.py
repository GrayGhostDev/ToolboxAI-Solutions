...
def test_imports() -> bool:
    ...
    try:
        from core.agents import (
            BaseAgent, AgentConfig, AgentState, TaskResult, AgentStatus, AgentPriority,
            TaskType, SupervisorDecision,
            ReviewSeverity, ReviewFinding,
            TestType, TestStatus, TestResult, TestSuiteResult,
            OrchestrationRequest, OrchestrationResult, WorkflowType,
            create_orchestrator, create_agent, get_available_agents
        )
...
def test_websocket_endpoints() -> bool:
    ...
    try:
        ...
        try:
            ws_routes = [(i, route.path) for i, route in enumerate(app.routes) if '/ws' in str(route.path)]
            ...
            if native_index is not None and param_index is not None:
                if native_index < param_index:
                    print(f"✅ Route ordering correct: /ws/native (index {native_index}) before /ws/{{client_id}} (index {param_index})")
                    test_results.append(True)
                else:
                    print(f"❌ Route ordering incorrect: /ws/native (index {native_index}) should be before /ws/{{client_id}} (index {param_index})")
                    test_results.append(False)
            else:
                print(f"⚠️  Could not verify route ordering (native: {native_index}, param: {param_index})")
