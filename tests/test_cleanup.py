...
class TestCleanupManager:
    ...
    def _generate_cleanup_report(self, stats: Dict[str, int]):
        ...
        report_file = self.test_log_dir / "reports" / f"cleanup_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def archive_old_reports(self, archive_dir: Optional[Path] = None):
...
def main():
    ...
    parser.add_argument("--archive", action="store_true", help="Archive old reports instead of deleting")
