...
class FlaskBridgeIntegrationTest:
    ...
    def test_plugin_registration(self):
        ...
        plugin_data = {"studio_id": "test-studio-123", "port": 64989, "version": "1.0.0"}

        try:
            response = requests.post(
                f"{self.flask_url}/register_plugin", json=plugin_data, timeout=5
            )
            ...
            if response.status_code == 200:
                ...
                heartbeat_response = requests.post(
                    f"{self.flask_url}/plugin/{plugin_id}/heartbeat", timeout=5
                )
                ...
                self.test_results.append(
                    {"test": "plugin_registration", "success": True, "plugin_id": plugin_id}
                )
                return True
            else:
                print(f"  ❌ Plugin registration failed: {response.status_code}")
                self.test_results.append(
                    {"test": "plugin_registration", "success": False, "error": response.text}
                )
                return False
            ...
            self.test_results.append(
                {"test": "plugin_registration", "success": False, "error": str(e)}
            )
            return False
    ...
    def test_content_generation_bridge(self):
        ...
        try:
            ...
            response = requests.post(
                f"{self.flask_url}/generate_simple_content", json=content_request, timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                if data.get("success"):
                    ...
                    if (
                        "SPARC" in data.get("message", "")
                        or "agent" in data.get("message", "").lower()
                    ):
            ...
            self.test_results.append(
                {"test": "content_generation_bridge", "success": False, "error": str(e)}
            )
            return False
    ...
    def test_script_templates(self):
        ...
        for script_type in script_types:
            try:
                response = requests.get(f"{self.flask_url}/script/{script_type}", timeout=5)
    ...
    def test_cache_functionality(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.test_results.append(
                    {"test": "cache_functionality", "success": True, "cache_stats": cache_stats}
                )
                return True
            else:
                print(f"  ❌ Failed to get cache stats: {response.status_code}")
                self.test_results.append({"test": "cache_functionality", "success": False})
                return False

        except Exception as e:
            print(f"  ❌ Cache functionality error: {e}")
            self.test_results.append(
                {"test": "cache_functionality", "success": False, "error": str(e)}
            )
            return False
    ...
    def test_agent_integration(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.test_results.append({"test": "agent_integration", "success": False})
                return False

        except Exception as e:
            print(f"  ❌ Agent integration check error: {e}")
            self.test_results.append(
                {"test": "agent_integration", "success": False, "error": str(e)}
            )
            return False
