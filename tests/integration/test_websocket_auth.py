...
class TestFastAPIWebSocketAuthenticator:
    ...
    def test_session_management(self, authenticator):
        ...
        # Create mock session using the dataclass version
        mock_websocket = Mock()
        ...
        assert session.token_payload is not None

        # Test session serialization using dataclass fields
        assert session.user_id == "test-user"
        assert session.username == "testuser"
        assert session.role == "student"
    ...
    async def test_authenticated_connection_flow(self):
        ...
        with patch('apps.backend.api.auth.websocket_auth.FastAPIWebSocketAuthenticator.authenticate_connection') as mock_auth:
            ...
            with patch('apps.backend.services.websocket_handler.WebSocketManager.connect') as mock_connect:
                ...
                with patch('apps.backend.api.auth.websocket_auth.authenticate_websocket_message') as mock_auth_msg:
