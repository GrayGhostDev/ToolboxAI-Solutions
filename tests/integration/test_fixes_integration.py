...
def test_authentication_endpoints():
    ...
    try:
        ...
        results["main_login"] = {
            ...
            "has_token": (
                "access_token" in response.json() if response.status_code == 200 else False
            ),
        }
        ...
        for name, endpoint in dashboard_endpoints:
            try:
                response = requests.get(
                    f"http://127.0.0.1:8001{endpoint}", headers=headers, timeout=5
                )
...
def test_cors_and_cross_service():
    ...
    try:
        response = requests.get(
            "http://127.0.0.1:8008/health", headers={"Origin": "http://localhost:3000"}, timeout=5
        )
