...
async def test_enhanced_content_agent():
    ...
    try:
        ...
        result = await content_agent.execute(
            task="Generate comprehensive programming lesson for 7th grade",
        ...
        if result.success:
            ...
            if "content" in content_data:
                ...
                logger.info(f"  - Standards alignment: {'✓' if content.get('standards_alignment') else '✗'}")
            ...
            if content_data.get("sparc_analysis"):
                ...
                logger.info(f"  - SPARC cycle: {'✓ Completed' if sparc_result.get('sparc_cycle_completed') else '✗ Failed'}")
        ...
        if vocabulary:
            ...
            for i, term in enumerate(vocabulary[:3]):  # Show first 3 terms
                logger.info(f"  - {term.get('term', 'unknown')}: {term.get('definition', 'no definition')[:100]}...")
        else:
        ...
        try:
            ...
            prompt = content_agent._build_content_generation_prompt(
                "Science", "6", "Solar System", "Learn about planets", "NGSS standards", enhanced_context
            )
        ...
        test_results = {
            ...
            "Database Integration": "✓ Passed" if content_agent.database_available else "ℹ Mock Mode",
        ...
        logger.info(f"- OpenAI API Key: {'Available' if os.getenv('OPENAI_API_KEY') else 'Not Available'}")
    ...
    try:
        ...
        if result.success:
            ...
            quality_indicators = [
                ...
                ("plant" in content_text.lower() or "leaf" in content_text.lower(), "Scientific accuracy"),
                ("sunlight" in content_text.lower() or "sun" in content_text.lower(), "Key concepts"),
