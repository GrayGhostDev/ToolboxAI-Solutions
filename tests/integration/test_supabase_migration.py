        ...
        'tables': [
            {
                ...
                'indexes': [
                    ...
                    {'name': 'idx_users_organization_id', 'column_names': ['organization_id'], 'unique': False}
                ],
                'unique_constraints': [
            ...
            },
            {
                ...
                'columns': [
                    ...
                    {'name': 'subscription_plan', 'type': 'VARCHAR(50)', 'nullable': False, 'default': 'free'},
                    {'name': 'created_at', 'type': 'TIMESTAMP', 'nullable': False, 'default': 'NOW()'},
                    {'name': 'updated_at', 'type': 'TIMESTAMP', 'nullable': False, 'default': 'NOW()'}
                ],
                'primary_key': {'constrained_columns': ['id']},
            ...
            },
            {
                ...
                'columns': [
                    ...
                    {'name': 'difficulty_level', 'type': 'INTEGER', 'nullable': False, 'default': '1'},
                    ...
                    {'name': 'status', 'type': 'content_status', 'nullable': False, 'default': 'draft'},
                ...
                'indexes': [
                    {'name': 'idx_lessons_organization_id', 'column_names': ['organization_id'], 'unique': False},
                    {'name': 'idx_lessons_created_by', 'column_names': ['created_by'], 'unique': False},
            ...
            },
            {
                ...
                'columns': [
                    ...
                    {'name': 'progress_percentage', 'type': 'INTEGER', 'nullable': False, 'default': '0'},
                ...
                'indexes': [
                    {'name': 'idx_enrollments_student_id', 'column_names': ['student_id'], 'unique': False},
                    {'name': 'idx_enrollments_lesson_id', 'column_names': ['lesson_id'], 'unique': False}
                ],
                'unique_constraints': [
                    {'name': 'uq_enrollments_student_lesson', 'column_names': ['student_id', 'lesson_id']}
                ],
                'check_constraints': [
                    {'name': 'ck_enrollments_progress', 'sqltext': 'progress_percentage BETWEEN 0 AND 100'}
                ],
                'row_count': 150000,
                'size_bytes': 20971520
            }
...
class TestSupabaseMigrationIntegration:
    ...
    async def test_full_migration_workflow(
        ...
        with patch.object(agent.schema_analyzer, 'analyze', new_callable=AsyncMock) as mock_analyze, \
             patch.object(agent.schema_analyzer, 'extract_relationships', new_callable=AsyncMock) as mock_relationships, \
             patch.object(agent.rls_generator, 'generate_policies', new_callable=AsyncMock) as mock_policies, \
            ...
            mock_relationships.return_value = self._extract_relationships_from_schema(sample_production_schema)
            ...
            migration_plan = await agent.generate_migration_plan(
                analysis_results,
                {'migration_type': 'full', 'preserve_data': True}
            )
    ...
    @pytest.mark.asyncio
    async def test_schema_analysis_integration(self, mock_database_connections, sample_production_schema):
    ...
    @pytest.mark.asyncio
    async def test_rls_policy_generation_integration(self, sample_production_schema, sample_access_patterns):
    ...
    @pytest.mark.asyncio
    async def test_data_migration_integration(self, mock_database_connections, sample_production_schema):
        ...
        with patch('asyncpg.connect', return_value=mock_conn):
            ...
            with patch.object(tool, 'migrate_table', new_callable=AsyncMock) as mock_migrate:
                ...
                def get_migration_result(source, target, table_name, **kwargs):
                    ...
                    from core.agents.supabase.tools.data_migration import MigrationResult, MigrationStatus
                ...
                assert all(table in results for table in ['organizations', 'users', 'lessons', 'enrollments'])
    ...
    @pytest.mark.asyncio
    async def test_migration_with_complex_dependencies(self, mock_database_connections, sample_production_schema):
    ...
    async def test_migration_validation_comprehensive(self, mock_database_connections):
        ...
        with patch.object(agent, '_validate_schema', new_callable=AsyncMock) as mock_schema_val, \
    ...
    async def test_migration_with_large_dataset_simulation(self, mock_database_connections):
        ...
        with patch.object(tool, '_get_row_count', new_callable=AsyncMock) as mock_count, \
