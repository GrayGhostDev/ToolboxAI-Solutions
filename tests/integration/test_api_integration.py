...
class TestPluginWorkflow:
    ...
    def test_plugin_registration_and_heartbeat_workflow(self, client):
        ...
        # 1. Register plugin
        plugin_data = {"studio_id": "integration-test-studio", "port": 64989, "version": "1.0.0"}
    ...
    def test_rate_limiting(self, client):
        ...
        plugin_data = {"studio_id": "rate-test-studio", "port": 64989}
        response = client.post("/register_plugin", json=plugin_data)
        ...
        with patch(
            "apps.backend.core.security.rate_limit_manager.RateLimitManager.check_rate_limit",
            ...
            assert (
                rate_limited_count > 0
            ), f"Expected rate limiting to trigger, but got {rate_limited_count} rate limited responses"
...
class TestContentGeneration:
    ...
    def test_terrain_generation(self, mock_post, client):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"success": True, "terrain_data": {"type": "forest"}}
        mock_post.return_value = mock_response

        request_data = {"theme": "forest", "size": "medium", "biome": "temperate"}

        response = client.post("/generate_terrain", json=request_data)
    ...
    def test_script_templates(self, client):
        ...
        for script_type, expected_text in script_expectations.items():
            ...
            assert (
                expected_text in response_text
            ), f"Expected '{expected_text}' in {script_type} template, but got: {response_text}"
...
class TestCacheOperations:
    ...
    def test_content_caching(self, mock_post, client):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"success": True, "content": {"test": "data"}}
        mock_post.return_value = mock_response
        ...
        ), f"Expected at most 2 calls (auth + content), got {mock_post.call_count}"
...
class TestErrorHandling:
    ...
    def test_invalid_json_requests(self, client):
        ...
        response = client.post(
            "/register_plugin", data="invalid json", headers={"Content-Type": "application/json"}
        )
