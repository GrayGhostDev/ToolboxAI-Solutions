...
class TestAdvancedSupervisorAgent:
    ...
    async def supervisor(self):
        ...
        config = AgentConfig(name="TestSupervisor", model="gpt-4-turbo-preview", temperature=0.1)
    ...
    async def test_agent_health_monitoring(self, supervisor):
        ...
        print(
            f"Health Report: {json.dumps(health_report, indent=2, default=make_json_serializable)}"
        )
    ...
    async def test_performance_monitoring(self, supervisor):
        ...
        print(
            f"Performance Report: {json.dumps(performance_report, indent=2, default=make_json_serializable)}"
        )
        ...
        with patch.object(AdvancedSupervisorAgent, "_build_workflow_graph") as mock_build:
            ...
            try:
                ...
                successful_workflows = sum(
                    ...
                    for result in results
                    if isinstance(result, WorkflowExecution)
                    and result.status == WorkflowStatus.COMPLETED
                )
...
if __name__ == "__main__":
    ...
    pytest.main(
        [
            ...
            "--tb=short",
            "-k",
            "test_educational_content_workflow or test_agent_health_monitoring",
        ]
    )
