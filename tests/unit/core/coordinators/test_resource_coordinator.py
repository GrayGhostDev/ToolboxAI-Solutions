...
class TestResourceAllocation:
    ...
    async def test_allocate_resources_success(self, initialized_coordinator):
        ...
        with patch.object(
            ...
            allocation = await initialized_coordinator.allocate_resources(
                "test_req_1", requirements
            )
...
class TestResourceRelease:
    ...
    async def test_release_resources_records_metrics(
        self, initialized_coordinator, sample_allocation, sample_usage
    ):
        ...
        with patch.object(
            initialized_coordinator, "_record_usage_metrics", new_callable=AsyncMock
        ) as mock_record:
...
class TestCostTracking:
    ...
    async def test_record_usage_metrics(
        self, initialized_coordinator, sample_allocation, sample_usage
    ):
    ...
    async def test_calculate_efficiency_score(
        self, initialized_coordinator, sample_allocation, sample_usage
    ):
...
class TestOptimization:
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    mock_api.return_value = {"wasted_quota": 25.0, "efficiency": 75.0}
                    mock_cost.return_value = {"potential_daily_savings": 6.50}
    ...
    async def test_analyze_allocation_patterns(self, initialized_coordinator, sample_allocation):
        ...
        initialized_coordinator.allocations["test_req_1"] = sample_allocation
        initialized_coordinator.usage_tracking["test_req_1"] = ResourceUsage(
            request_id="test_req_1", cpu_usage=1.5, memory_usage_mb=1500
        )
    ...
    async def test_analyze_api_efficiency(
        self, initialized_coordinator, sample_allocation, sample_usage
    ):
...
class TestBackgroundTasks:
    ...
    async def test_cleanup_expired_allocations(self, initialized_coordinator):
        ...
        initialized_coordinator.usage_tracking["expired_req"] = ResourceUsage(
            request_id="expired_req"
        )
    ...
    async def test_get_health_unhealthy(self, initialized_coordinator):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                mock_alerts.return_value = [
                    "High CPU utilization: 97.0%",
                    "High memory utilization: 96.0%",
                ]
...
class TestContextManager:
    ...
    async def test_resource_context_allocates_and_releases(self, initialized_coordinator):
        ...
        with patch.object(
            initialized_coordinator, "allocate_resources", new_callable=AsyncMock
        ) as mock_allocate:
            with patch.object(
                initialized_coordinator, "release_resources", new_callable=AsyncMock
            ) as mock_release:
                mock_allocate.return_value = ResourceAllocation(
                    request_id="ctx_test", cpu_cores=1, memory_mb=512
                )

                async with initialized_coordinator.resource_context(
                    "ctx_test", requirements
                ) as allocation:
...
class TestShutdownAndCleanup:
    ...
    async def test_shutdown_releases_all_allocations(self, initialized_coordinator):
        ...
        initialized_coordinator.allocations["req_1"] = ResourceAllocation(
            request_id="req_1", cpu_cores=1, memory_mb=512
        )
        initialized_coordinator.allocations["req_2"] = ResourceAllocation(
            request_id="req_2", cpu_cores=2, memory_mb=1024
        )
