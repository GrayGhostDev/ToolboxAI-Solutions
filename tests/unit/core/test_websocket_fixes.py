...
def fix_websocket_auth():
    ...
    def mock_decode_token(token, *args, **kwargs):
        if token == valid_token:
            return {"sub": str(uuid.uuid4()), "username": "test_user", "role": "student"}
...
def fix_rate_limit_override():
    ...
    test_config = RateLimitConfig(max_requests=100, window_seconds=60, burst_size=10)

    with patch(
        "apps.backend.core.security.rate_limit_manager.get_rate_limit_config",
