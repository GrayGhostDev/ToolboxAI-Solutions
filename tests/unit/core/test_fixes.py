...
def fix_websocket_tests():
    ...
    with patch('apps.backend.services.websocket_handler.decode_access_token', return_value=mock_user):
...
def fix_security_tests():
    ...
    with patch('apps.backend.core.security.input_validation.validate_file_upload', return_value=True):
        with patch('apps.backend.core.security.sql_injection.check_sql_injection', return_value=False):
            yield
