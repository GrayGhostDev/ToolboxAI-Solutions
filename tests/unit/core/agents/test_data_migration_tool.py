...
def migration_tool():
    ...
    return DataMigrationTool(batch_size=100, max_concurrent_batches=3, retry_attempts=2)
...
class TestDataMigrationTool:
    ...
    def test_tool_initialization(self):
        ...
        tool = DataMigrationTool(batch_size=500, max_concurrent_batches=10, retry_attempts=5)
    ...
    async def test_migrate_table_dry_run(self, migration_tool, mock_asyncpg_connection):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            ...
            with patch.object(
                migration_tool, "_validate_schema_compatibility", new_callable=AsyncMock
            ) as mock_validate:
                mock_validate.return_value = True

                result = await migration_tool.migrate_table(
                    "postgresql://source", "postgresql://target", "test_table", dry_run=True
                )
    ...
    async def test_migrate_table_actual(
        self, migration_tool, mock_asyncpg_connection, sample_table_data
    ):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            ...
            with (
                patch.object(
                    migration_tool, "_extract_batch_data", new_callable=AsyncMock
                ) as mock_extract,
                patch.object(
                    migration_tool, "_load_batch_data", new_callable=AsyncMock
                ) as mock_load,
            ):
                ...
                result = await migration_tool.migrate_table(
                    "postgresql://source", "postgresql://target", "test_table", dry_run=False
                )
    ...
    @pytest.mark.asyncio
    async def test_migrate_table_with_progress_callback(
        self, migration_tool, mock_asyncpg_connection
    ):
    ...
    async def test_migrate_table_failure(self, migration_tool, mock_asyncpg_connection):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            ...
            result = await migration_tool.migrate_table(
                "postgresql://invalid", "postgresql://target", "test_table", dry_run=False
            )
    ...
    async def test_migrate_database(self, migration_tool, mock_asyncpg_connection):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            ...
            with patch.object(
                migration_tool, "migrate_table", new_callable=AsyncMock
            ) as mock_migrate:
                ...
                results = await migration_tool.migrate_database(
                    "postgresql://source", "postgresql://target", dry_run=True
                )
    ...
    async def test_incremental_migration(self, migration_tool, mock_asyncpg_connection):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            ...
            with patch.object(
                migration_tool, "_execute_incremental_migration", new_callable=AsyncMock
            ) as mock_execute:
                ...
                result = await migration_tool.incremental_migration(
                    "postgresql://source", "postgresql://target", "test_table", "updated_at"
                )
    ...
    async def test_rollback_migration(self, migration_tool, mock_asyncpg_connection):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            with (
                patch.object(migration_tool, "_clear_table", new_callable=AsyncMock) as mock_clear,
                patch.object(
                    migration_tool, "_restore_from_backup", new_callable=AsyncMock
                ) as mock_restore,
            ):

                success = await migration_tool.rollback_migration(
                    "postgresql://target", "test_table", "/path/to/backup.json"
                )
    ...
    async def test_validate_migration_basic(self, migration_tool, mock_asyncpg_connection):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            ...
            result = await migration_tool.validate_migration(
                "postgresql://source", "postgresql://target", "test_table", "basic"
            )
    ...
    async def test_validate_migration_full_mode(self, migration_tool, mock_asyncpg_connection):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            ...
            with patch.object(
                migration_tool, "_validate_sample_data", new_callable=AsyncMock
            ) as mock_sample:
                mock_sample.return_value = {"sample_size": 100, "mismatches": 0, "accuracy": 1.0}

                result = await migration_tool.validate_migration(
                    "postgresql://source", "postgresql://target", "test_table", "full"
                )
    ...
    async def test_get_row_count(self, migration_tool, mock_asyncpg_connection):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            ...
            count = await migration_tool._get_row_count("postgresql://test", "test_table")
    ...
    async def test_get_row_count_with_where_clause(self, migration_tool, mock_asyncpg_connection):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            ...
            count = await migration_tool._get_row_count(
                "postgresql://test", "test_table", "created_at > '2023-01-01'"
            )

            assert count == 750

    @pytest.mark.asyncio
    async def test_extract_batch_data(
        self, migration_tool, mock_asyncpg_connection, sample_table_data
    ):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            ...
            with patch("builtins.dict", side_effect=sample_table_data):
                data = await migration_tool._extract_batch_data("postgresql://source", batch)

                assert len(data) == 3
                assert data == sample_table_data

    @pytest.mark.asyncio
    async def test_load_batch_data(
        self, migration_tool, mock_asyncpg_connection, sample_table_data
    ):
        """Test batch data loading."""
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            success = await migration_tool._load_batch_data(
                "postgresql://target", "test_table", sample_table_data
            )
    ...
    async def test_load_batch_data_empty(self, migration_tool, mock_asyncpg_connection):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            success = await migration_tool._load_batch_data("postgresql://target", "test_table", [])

            assert success is True
            mock_asyncpg_connection.executemany.assert_not_called()

    @pytest.mark.asyncio
    async def test_load_batch_data_failure(
        self, migration_tool, mock_asyncpg_connection, sample_table_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_schema_compatibility_success(
        self, migration_tool, mock_asyncpg_connection
    ):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            ...
            with patch("builtins.dict", side_effect=schema_data * 2):  # Source and target schemas
                result = await migration_tool._validate_schema_compatibility(
                    "postgresql://source", "postgresql://target", "test_table"
                )

                assert result is True

    @pytest.mark.asyncio
    async def test_validate_schema_compatibility_missing_table(
        self, migration_tool, mock_asyncpg_connection
    ):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            ...
            with patch("builtins.dict", side_effect=[source_schema[0]]):
                with patch.object(
                    migration_tool, "_get_table_schema", new_callable=AsyncMock
                ) as mock_schema:
    ...
    @pytest.mark.asyncio
    async def test_process_single_batch_success(
        self, migration_tool, sample_migration_batch, sample_table_data
    ):
        ...
        with (
            ...
            success = await migration_tool._process_single_batch(
                "postgresql://source", "postgresql://target", sample_migration_batch
            )
    ...
    async def test_validate_sample_data(self, migration_tool, mock_asyncpg_connection):
        ...
        with patch("asyncpg.connect", return_value=mock_asyncpg_connection):
            ...
            # Mock sample IDs
            mock_asyncpg_connection.fetch.return_value = [{"id": 1}, {"id": 2}]

            result = await migration_tool._validate_sample_data(
                "postgresql://source", "postgresql://target", "test_table", 2
            )
...
class TestMigrationResult:
    ...
    def test_migration_result_with_validation(self):
        ...
        validation_data = {"is_valid": True, "issues": [], "metrics": {"accuracy": 0.99}}
