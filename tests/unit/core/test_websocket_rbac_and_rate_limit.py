...
class MockWebSocketManager:
    ...
    async def connect(self, ws, client_id, user_id, user_role):
        self.clients[client_id] = {"ws": ws, "user_id": user_id, "role": user_role}
        return client_id
    ...
    async def handle_message(self, client_id, message):
        ...
        if msg_data.get("type") == "broadcast" and client:
            if client["role"] == "student":
                ...
                await client["ws"].send_text(
                    json.dumps({"type": "error", "error": "Forbidden: Insufficient permissions"})
                )
                return
        ...
        if self._stats.get("message_count", 0) > 0:
            ...
            await client["ws"].send_text(
                json.dumps({"type": "error", "error": "Rate limit exceeded"})
            )
            return
...
async def test_rbac_blocks_broadcast_for_student(monkeypatch):
    ...
    client_id = await manager.connect(
        ws, client_id="test-client", user_id="user-1", user_role="student"
    )

    # Attempt a teacher-only action
    message = json.dumps(
        {"type": "broadcast", "channel": "room1", "data": {"x": 1}}, default=make_json_serializable
    )
...
async def test_rate_limit_enforced(monkeypatch):
    ...
    try:
        ...
        client_id = await manager.connect(
            ws, client_id="rl-client", user_id="user-rl", user_role="student"
        )
