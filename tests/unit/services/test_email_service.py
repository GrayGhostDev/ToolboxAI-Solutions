...
def mock_bleach():
    ...
    mock_bleach_module.clean = MagicMock(
                                ...
                                with patch.object(email_service, "FileName", MagicMock):
                                    with patch.object(email_service, "FileType", MagicMock):
                                        with patch.object(email_service, "Disposition", MagicMock):
                                            with patch(
                                                "apps.backend.services.email_service.EmailService._create_sendgrid_client",
...
def mock_template_env():
    ...
    with patch("apps.backend.services.email_service.Environment") as mock:
        ...
        mock_template.render_async = AsyncMock(return_value="<html>Test HTML</html>")
...
class TestEmailService:
    ...
    @pytest.mark.asyncio
    async def test_send_subscription_confirmation_email(
        self, service, mock_sendgrid, mock_template_env
    ):
    ...
    async def test_send_email_error_handling(self, service, mock_sendgrid):
        ...
        with pytest.raises(Exception) as exc:
            await service.send_email(
                to_emails="user@example.com", subject="Test Error", html_content="<p>Test</p>"
            )
    ...
    async def test_provider_not_configured(self, mock_settings):
        ...
        with pytest.raises(ValueError) as exc:
            await service.send_email(
                to_emails="user@example.com", subject="Test", html_content="<p>Test</p>"
            )
...
class TestEmailTemplates:
    ...
    async def test_render_template(self, service, mock_template_env):
        ...
        mock_html_template.render_async = AsyncMock(return_value="<html>Hello John</html>")
        mock_text_template = MagicMock()
        mock_text_template.render_async = AsyncMock(return_value="Hello John")

        mock_template_env.get_template.side_effect = [mock_html_template, mock_text_template]

        # Render template
        html, text = await service._render_template("welcome", {"user_name": "John"})
