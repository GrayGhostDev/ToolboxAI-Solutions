...
def mock_sendgrid_client():
    ...
    mock_client.client.asm.groups._.return_value.suppressions.post.return_value = MagicMock(
        status_code=201
    )
    ...
    with patch("apps.backend.services.email_service_sendgrid.HTML_PROCESSING_AVAILABLE", True):
        with patch("apps.backend.services.email_service_sendgrid.bleach") as mock_bleach:
            ...
            with patch("apps.backend.services.email_service_sendgrid.BeautifulSoup") as mock_bs:
                ...
                with patch(
                    "apps.backend.services.email_service_sendgrid.transform"
                ) as mock_transform:
...
async def service(
    ...
    with patch("apps.backend.services.email_service_sendgrid.SENDGRID_AVAILABLE", True):
        with patch(
            "apps.backend.services.email_service_sendgrid.SendGridAPIClient"
...
class TestSendGridEmailService:
    ...
    async def test_send_email_with_attachments(self, service):
        ...
        attachments = [
            EmailAttachment(
                filename="test.pdf", content=b"PDF content here", type="application/pdf"
            ),
            EmailAttachment(
                filename="image.png",
    ...
    async def test_send_email_with_custom_args(self, service):
        ...
        result = await service.send_email(
            ...
            html_content="<p>Content</p>",
            custom_args={"campaign_id": "summer-2025", "user_segment": "premium"},
        )
    ...
    async def test_handle_sendgrid_error(self, service):
        ...
        result = await service.send_email(
            to_emails="invalid-email", subject="Test", html_content="<p>Test</p>"
        )
    ...
    async def test_add_to_suppression_list(self, service):
        ...
        result = await service.add_to_suppression_list(
            email="bounced@example.com", suppression_type="bounce"
        )
    ...
    async def test_remove_from_suppression_list(self, service):
        ...
        service.client.client.suppression.bounces._.return_value.delete.return_value = MagicMock(
            status_code=204
        )
...
class TestEmailAttachment:
    ...
    def test_email_attachment_defaults(self):
        ...
        attachment = EmailAttachment(filename="image.png", content=b"PNG data", type="image/png")
