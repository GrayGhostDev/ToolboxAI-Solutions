...
class TestSecurityCompliance:
    ...
    def test_no_secrets_in_code(self):
        ...
        if result.returncode != 0:
            ...
            if "FileNotFoundError" in result.stderr:
                pytest.skip("No secrets baseline found - run 'detect-secrets scan --update .secrets.baseline' first")
            else:
    ...
    def test_python_code_security(self):
        ...
        if result.stdout:
            try:
                ...
                high_severity = [i for i in report.get("results", []) if i.get("issue_severity") == "HIGH"]
        ...
        for env_file in env_files:
            ...
            if file_path.exists():
                ...
                for pattern in dangerous_patterns:
                    ...
                    assert len(matches) == 0, f"Potential real API key found in {env_file}: {matches}"

                # Check for placeholder patterns are used
                placeholders = ['your-key', 'your-secret', 'your-token', 'xxx', 'placeholder']
                api_key_lines = [line for line in content.split('\n') if 'API_KEY' in line or 'SECRET' in line or 'TOKEN' in line]

                for line in api_key_lines:
                    if '=' in line and not line.strip().startswith('#'):
                        value = line.split('=', 1)[1].strip().strip('"\'')
                        if value and not any(placeholder in value.lower() for placeholder in placeholders):
    ...
    def test_pusher_security_configuration(self):
        ...
        assert hasattr(PusherService, 'trigger_event'), "PusherService should have trigger_event method"
        ...
        for env_file in env_files:
            ...
            if file_path.exists():
                ...
                for dangerous_config in dangerous_db_configs:
                    assert dangerous_config not in content, f"Dangerous database config found in {env_file}"

    def test_cors_configuration_security(self):
        """Test CORS configuration is not overly permissive"""
        backend_main = Path("/Volumes/G-DRIVE ArmorATD/Development/Clients/ToolBoxAI-Solutions/apps/backend/main.py")
                ...
                if matches:
                    ...
                    for i, line in enumerate(context_lines):
                        if '*' in line and 'origin' in line.lower():
                            ...
                            if not any(keyword in context.lower() for keyword in ['development', 'debug', 'dev', 'test']):
    ...
    def test_jwt_security_configuration(self):
        ...
        # Check JWT settings
        settings_file = Path("/Volumes/G-DRIVE ArmorATD/Development/Clients/ToolBoxAI-Solutions/toolboxai_settings/settings.py")
                ...
                for line in lines:
                    if 'SECRET_KEY' in line and '=' in line:
                        ...
                        for weak_key in weak_keys:
                            assert weak_key.lower() not in value.lower(), f"Weak JWT secret key found: {line}"

    def test_file_upload_security(self):
        """Test file upload security measures"""
        # Check for file upload handlers and security measures
        backend_files = Path("/Volumes/G-DRIVE ArmorATD/Development/Clients/ToolBoxAI-Solutions/apps/backend").rglob("*.py")
    ...
    def test_sql_injection_prevention(self):
        ...
        python_files = Path("/Volumes/G-DRIVE ArmorATD/Development/Clients/ToolBoxAI-Solutions").rglob("*.py")
        ...
        for file_path in python_files:
            ...
            try:
                ...
                for pattern in dangerous_sql_patterns:
                    matches = re.findall(pattern, content)
                    if matches:
                        pytest.fail(f"Potential SQL injection vulnerability in {file_path}: {matches}")
