...
class TestComprehensiveSecurity:
                ...
                try:
                    ...
                    for pattern in secret_patterns:
                        ...
                        for match in matches:
                            ...
                            if any(fp in match_text.lower() for fp in ['example', 'test', 'mock', 'placeholder', 'dummy']):
                                continue
    ...
    def test_dependency_vulnerabilities(self):
        ...
        try:
            result = subprocess.run(
                ["safety", "check", "--json", "--ignore", "70612"],  # Ignore Jinja2 issue if present
                capture_output=True,
                text=True,
                timeout=60
            )
            ...
            if result.returncode != 0 and result.stdout:
                try:
                    ...
                    assert len(critical) <= 3, f"Found {len(critical)} critical vulnerabilities (max 3 allowed)"
    ...
    def test_sql_injection_protection(self):
        ...
        for py_file in project_root.rglob('*.py'):
            ...
            if any(excluded in str(py_file) for excluded in ['test_', '__pycache__', '.git', 'node_modules']):
                continue
            ...
            try:
                ...
                for pattern in sql_patterns:
                    ...
                    for match in matches:
                        ...
                        if any(safe in line_content.lower() for safe in ['%s', '?', ':param', 'text(', 'bindparam']):
                            continue
        ...
        assert len(issues) == 0, f"Potential SQL injection vulnerabilities found in: {[i['file'] for i in issues]}"
    ...
    def test_xss_protection(self):
        ...
        for file_pattern in ['*.tsx', '*.ts', '*.js', '*.jsx', '*.html', '*.vue']:
            for file in project_root.rglob(file_pattern):
                ...
                if any(excluded in str(file) for excluded in ['node_modules', 'test_', '__tests__', '.test.', '.spec.']):
                    continue
                ...
                try:
                    ...
                    for pattern in xss_patterns:
                        ...
                        for match in matches:
                            ...
                            if any(sanitizer in surrounding.lower() for sanitizer in ['dompurify', 'sanitize', 'escape', 'htmlescape']):
                                continue
        ...
        assert len(issues) == 0, f"Potential XSS vulnerabilities found in: {[i['file'] for i in issues]}"
            ...
            try:
                ...
                for pattern in jwt_patterns:
                    ...
                    for match in matches:
                        ...
                        if len(secret) < 32:
                            issues.append(f"JWT/Auth secret too short in {env_file.name}: {len(secret)} chars (min 32)")
                        if secret.lower().startswith(('change', 'default', 'test', 'secret')):
                            issues.append(f"JWT/Auth secret uses insecure default value in {env_file.name}")
    ...
    def test_cors_configuration(self):
            ...
            try:
                ...
                if 'CORSMiddleware' in content or 'cors' in content.lower():
                    ...
                    if 'allow_origins=["*"]' in content or "allow_origins=['*']" in content:
                        ...
                        if not ('development' in content.lower() or 'debug' in content.lower() or 'env' in content.lower()):
        ...
        assert len(wildcard_issues) == 0, f"Wildcard CORS without environment check in: {wildcard_issues}"
    ...
    def test_rate_limiting(self):
        ...
        backend_files = [f for f in backend_files if any(part in str(f) for part in ['backend', 'api', 'main', 'app'])]
        backend_files = [f for f in backend_files if not any(excluded in str(f) for excluded in ['test_', '__pycache__'])]
    ...
    def test_security_headers(self):
        ...
        backend_files = list(project_root.rglob('*.py'))
        backend_files = [f for f in backend_files if any(part in str(f) for part in ['backend', 'api', 'main', 'middleware'])]
        backend_files = [f for f in backend_files if not any(excluded in str(f) for excluded in ['test_', '__pycache__'])]
    ...
    def test_environment_variable_security(self):
        ...
        env_files = [f for f in env_files if f.is_file() and 'example' not in f.name and 'template' not in f.name]
            ...
            try:
                ...
                for pattern in env_patterns:
                    if re.search(pattern, content):
                        ...
                        if 'os.environ' not in content and 'getenv' not in content and 'config' not in content.lower():
                            issues.append(f"Potential hardcoded environment variable in {py_file.relative_to(project_root)}")
                            break
            except Exception:
                continue
        ...
        assert len([i for i in issues if 'gitignore' in i]) == 0, "Critical environment security issues found"
    ...
    def test_file_upload_security(self):
        ...
        for upload_file in upload_files:
            try:
                ...
                security_checks = {
                    'file_type_validation': any(check in content.lower() for check in ['content_type', 'mimetype', 'file_extension']),
                    'size_limit': any(check in content.lower() for check in ['size', 'max_size', 'limit']),
                    'path_traversal': any(check in content.lower() for check in ['secure_filename', 'sanitize', 'path']),
                }

                missing_checks = [check for check, present in security_checks.items() if not present]
                if missing_checks and len(content) > 500:  # Only check substantial files
                    issues.append(f"Upload file {upload_file.relative_to(project_root)} missing security checks: {missing_checks}")
