...
from apps.backend.api.auth.auth import JWTManager, hash_password
...
pytestmark = pytest.mark.skip(
    reason="Performance tests require external services - run with --run-performance"
)
...
TEST_DATABASE_URL = (
    "postgresql+asyncpg://eduplatform:eduplatform2024@localhost/educational_platform_dev"
)
...
class TestAPIv1Endpoints:
    ...
    def test_realtime_analytics_success(self):
        ...
        response = client.get(
            "/api/v1/analytics/realtime", headers={"Authorization": f"Bearer {self.admin_token}"}
        )
    ...
    def test_realtime_analytics_student_access(self):
        ...
        response = client.get(
            "/api/v1/analytics/realtime", headers={"Authorization": f"Bearer {self.student_token}"}
        )
    ...
    def test_list_users_unauthorized(self):
        ...
        response = client.get(
            "/api/v1/admin/users", headers={"Authorization": f"Bearer {self.student_token}"}
        )
    ...
    def test_analytics_to_report_workflow(self):
        ...
        summary_response = client.get(
            "/api/v1/analytics/summary", headers={"Authorization": f"Bearer {self.admin_token}"}
        )
    ...
    def test_user_management_complete_lifecycle(self):
        ...
        assert list_response.status_code == 200
        found_users = [
            u for u in list_response.json()["users"] if u["username"] == "lifecycle_user"
        ]
