...
class DocumentationValidator:
    ...
    def __init__(self):
        self.doc_path = Path(
            "/Volumes/G-DRIVE ArmorATD/Development/Clients/ToolBoxAI-Solutions/docs"
        )
        self.issues = []

    def validate_no_active_websocket_refs(self):
        """Ensure WebSocket references are marked as deprecated"""
        patterns = [r"ws://", r"wss://", r"new WebSocket", r"/ws/", r"socket\.io"]
    ...
    def validate_pusher_documentation(self):
        ...
        if missing:
            self.issues.append(
                {"issue": "Missing Pusher channel documentation", "channels": list(missing)}
            )
    ...
    def validate_api_documentation_consistency(self):
        ...
        if not api_spec_file.exists():
            self.issues.append(
                {"issue": "OpenAPI specification file not found", "file": str(api_spec_file)}
            )
            return False
        ...
        try:
            ...
            pusher_endpoints = ["/pusher/auth", "/api/v1/realtime/trigger", "/pusher/webhook"]
            ...
            self.issues.append(
                {"issue": f"Invalid JSON in API spec: {e}", "file": str(api_spec_file)}
            )
    ...
    def validate_environment_config_docs(self):
        ...
        if not env_example_file.exists():
            self.issues.append(
                {"issue": "Environment example file not found", "file": str(env_example_file)}
            )
            return False
    ...
    def validate_docker_config_docs(self):
        ...
        if not docker_compose_file.exists():
            self.issues.append(
                {"issue": "Docker compose file not found", "file": str(docker_compose_file)}
            )
            return False
...
if __name__ == "__main__":
    ...
    report_file = Path(
        "/Volumes/G-DRIVE ArmorATD/Development/Clients/ToolBoxAI-Solutions/tests/reports/documentation_validation_report.json"
    )
