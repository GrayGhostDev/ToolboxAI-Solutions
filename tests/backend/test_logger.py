...
class TestLogger:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        json_file = (
            reports_dir / f"structured_{self.name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def start_test(self, test_name: str, correlation_id: str | None = None):
    ...
    def end_test(
        self, test_name: str, status: str, duration: float = 0.0, error: str | None = None
    ):
    ...
    def generate_report(self):
        ...
        report_file = (
            TEST_LOG_DIR
            / "reports"
            / f"test_report_{self.name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
