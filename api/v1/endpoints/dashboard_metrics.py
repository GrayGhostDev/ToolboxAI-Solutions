...
from apps.backend.services.cache_service import cache_service, cached
from database.models.models import EducationalContent, User
...
async def calculate_user_metrics(session: AsyncSession, organization_id: UUID) -> UserMetrics:
    ...
    growth_rate = (
        (current_month_new / previous_month_total * 100) if previous_month_total > 0 else 0
    )
...
async def calculate_content_metrics(session: AsyncSession, organization_id: UUID) -> ContentMetrics:
...
async def get_detailed_statistics(
    ...
    return {
        ...
        "data": {
            ...
            "user_growth": [{"date": "2025-10-01", "new_users": 12, "total_users": 1234}],
            "content_creation": [{"date": "2025-10-01", "new_content": 5, "total_content": 890}],
            "engagement_trends": {
...
async def export_metrics(
    ...
    """
    logger.info(f"Exporting metrics in {format} format", extra={"user_id": str(current_user.id)})
