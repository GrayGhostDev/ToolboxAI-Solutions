...
class RobloxDeploymentManagerAgent(BaseAgent):
    ...
    def _check_staging_deployment(self, game_id: str, version: str) -> bool:
        ...
        for deployment in self.deployment_history:
            if (deployment.get("game_id") == game_id and
                deployment.get("version") == version and
                deployment.get("environment") == DeploymentEnvironment.STAGING.value and
                deployment.get("status") == DeploymentStatus.COMPLETED.value):
                return True
        return False
