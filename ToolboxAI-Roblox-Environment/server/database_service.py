        ...
        async with self.pool.acquire() as conn:
            ...
            return {
                ...
                "kpis": {
                    ...
                    "todaysLessons": len([l for l in lessons if 
                        datetime.fromisoformat(str(l['scheduled_at'])).date() == datetime.now().date()]),
                    "pendingAssessments": performance['pending_grading'] if performance else 0,
                    "averageProgress": float(performance['average_grade']) if performance and performance['average_grade'] else 0,
        ...
        async with self.pool.acquire() as conn:
            ...
            return {
                ...
                "kpis": {
                    "xp": student['xp_points'] or 0,
                    "level": student['level'] or 1,
                    "completedMissions": len([a for a in assignments if a['status'] == 'completed']),
        ...
        async with self.pool.acquire() as conn:
            ...
            return {
                ...
                "systemMetrics": {
                    ...
                    "avgResponseTime": float(api_metrics['avg_response_time']) if api_metrics and api_metrics['avg_response_time'] else 0,
                ...
                "compliance": {
                    "status": "Compliant" if compliance and compliance['pending_reviews'] == 0 else "Review Needed",
        ...
        async with self.pool.acquire() as conn:
            ...
            for child in children:
                ...
                child_data = {
                    ...
                    "attendanceRate": float(child['attendance_rate']) if child['attendance_rate'] else 95.0
                }
                ...
                if len(dashboard_data["children"]) == 1:
                    ...
                    dashboard_data["childProgress"] = {
                        "overallGrade": sum(float(g['average_grade']) for g in grades if g['average_grade']) / len(grades) if grades else 0,
                        "subjects": [
                            {
                                "name": g['subject'],
                    ...
                    dashboard_data["attendance"] = {
                        ...
                        "rate": (attendance['present'] / attendance['total'] * 100) if attendance and attendance['total'] > 0 else 95.0
                    }
