...
describe('Complete API Service Test Suite - Final', () => {
  ...
  describe('Authentication', () => {
    it('should successfully login with email and password', async () => {
      ...
      // The login method itself doesn't set localStorage, that's done by the calling component
      const requestData = JSON.parse(mock.history.post[0].data);
      expect(requestData.username).toBe('teacher'); // Username is extracted from email
      expect(requestData.password).toBe('SecurePass123!');
    });
    ...
    it('should successfully register a new user', async () => {
      ...
      // The register method transforms the response to have accessToken/refreshToken
    ...
    it('should refresh access token', async () => {
      ...
      expect(result).toEqual(mockResponse);
      const requestData = JSON.parse(mock.history.post[0].data);
      expect(requestData.refresh_token).toBe('test-refresh-token');
    });
    ...
    it('should successfully logout user', async () => {
      ...
      // The logout method itself doesn't clear localStorage
      // That's handled by the response interceptor or calling component
  ...
  describe('Assessments', () => {
    ...
    it('should create assessment', async () => {
      ...
      const requestData = JSON.parse(mock.history.post[0].data);
      expect(requestData.class_id).toBe('class-1'); // Should be snake_case
      expect(requestData.questions).toEqual([]); // Should include empty questions array
    });
    ...
    it('should publish assessment', async () => {
      ...
      mock.onPut(`${API_BASE_URL}/assessments/assess-123/publish`).reply(200, mockResponse);
  ...
  describe('Progress', () => {
    it('should get student progress', async () => {
      ...
      mock.onGet(`${API_BASE_URL}/progress/student/student-123`).reply(200, mockProgress);
    ...
    it('should update progress', async () => {
      ...
      // Check that params are sent correctly
  ...
  describe('Gamification', () => {
    ...
    it('should add XP', async () => {
      ...
      mock.onPost(`${API_BASE_URL}/gamification/xp/student-123`).reply(200, mockTransaction);

      const result = await apiClient.addXP('student-123', 50, 'Quiz completion');
      
      expect(result.amount).toBe(50);
      const requestData = JSON.parse(mock.history.post[0].data);
      expect(requestData).toEqual({ amount: 50, reason: 'Quiz completion' });
    });
    ...
    it('should award badge', async () => {
      ...
      const requestData = JSON.parse(mock.history.post[0].data);
      expect(requestData).toEqual({ studentId: 'student-123', badgeId: 'badge-2' });
    });
  ...
  describe('Messages', () => {
    ...
    it('should send message', async () => {
      ...
      const requestData = JSON.parse(mock.history.post[0].data);
      expect(requestData).toEqual(messageData);
    });
  ...
  describe('Compliance', () => {
    ...
    it('should record consent', async () => {
      ...
      const requestData = JSON.parse(mock.history.post[0].data);
      expect(requestData.consent_type).toBe('coppa'); // Should be snake_case
      expect(requestData.granted).toBe(true);
      expect(requestData.details.userId).toBe('user-123');
    });
  });
  ...
  describe('LMS Integration', () => {
    it('should connect Google Classroom', async () => {
      ...
      mock.onPost(`${API_BASE_URL}/integrations/google_classroom`).reply(200, mockResponse);

      const result = await apiClient.connectGoogleClassroom('google-token');
      
      expect(result.connected).toBe(true);
      const requestData = JSON.parse(mock.history.post[0].data);
      expect(requestData.token).toBe('google-token');
    });

    it('should connect Canvas', async () => {
      const mockResponse = { connected: true };

      mock.onPost(`${API_BASE_URL}/integrations/canvas`).reply(200, mockResponse);
