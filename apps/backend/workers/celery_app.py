...
from .config import get_monitoring_config, get_queue_definitions, get_worker_config
    ...
    if monitoring_config.get("prometheus", {}).get("enabled"):
        try:
            ...
            task_counter = Counter(
                "celery_tasks_total", "Total Celery tasks", ["task_name", "status", "tenant_id"]
            )
            task_duration = Histogram(
                "celery_task_duration_seconds", "Task duration", ["task_name", "tenant_id"]
            )
    ...
    dictConfig(
        {
            ...
            "root": {
                ...
                "handlers": [
                    h for h in ["console", "file"] if h in log_config.get("handlers", ["console"])
                ],
            },
            "loggers": {
                "celery": {"level": "INFO"},
...
def on_task_postrun(
    ...
    if hasattr(sender.app, "metrics"):
        sender.app.metrics["task_counter"].labels(
            task_name=sender.name, status=state.lower(), tenant_id=tenant_id
        ).inc()
