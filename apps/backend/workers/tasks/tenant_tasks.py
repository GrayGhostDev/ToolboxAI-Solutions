...
@shared_task(
    bind=True,
    name="tenant_tasks.generate_tenant_report",
    queue="tenant_operations",
    max_retries=2
)
    ...
    try:
        ...
        report_data['summary'] = {
            'key_metrics': {
                ...
                'system_health': "Good" if report_data['api_metrics']['error_rate'] < 0.05 else "Needs Attention"
            },
            'recommendations': [
...
@shared_task(
    bind=True,
    name="tenant_tasks.cleanup_tenant_data",
    queue="tenant_operations",
    max_retries=1
)
def cleanup_tenant_data(
    self,
    organization_id: str,
    cleanup_inactive_tenants: bool = True,
...
@shared_task(
    bind=True,
    name="tenant_tasks.sync_billing_data",
    queue="tenant_operations",
    max_retries=3
)
...
@shared_task(
    bind=True,
    name="tenant_tasks.track_usage_metrics",
    queue="analytics",
    max_retries=2
)
...
@shared_task(
    bind=True,
    name="tenant_tasks.backup_tenant_data",
    queue="tenant_operations",
    max_retries=1
)
...
@shared_task(
    bind=True,
    name="tenant_tasks.monitor_tenant_health",
    queue="default"
)
def monitor_tenant_health(
    ...
    try:
        ...
        logger.info(f"Health check completed for organization {organization_id}: {health_status['overall_status']}")
