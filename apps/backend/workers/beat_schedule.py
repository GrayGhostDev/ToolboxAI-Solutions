...
def get_beat_schedule() -> dict[str, Any]:
    ...
    return {
        ...
        "optimize-storage": {
            ...
            "kwargs": {"organization_id": None},  # Global optimization
        },
        "generate-monthly-storage-reports": {
            "task": "apps.backend.workers.tasks.storage_tasks.generate_storage_report",
            "schedule": timedelta(days=30),
            "options": {"queue": "analytics"},
            "kwargs": {"organization_id": None, "report_type": "monthly"},  # All organizations
        },
        # ========================================
        ...
        "cleanup-old-files": {
            ...
            "schedule": timedelta(hours=6),
            "options": {"queue": "low_priority"},
            "kwargs": {"directory": "/tmp/toolboxai", "days_old": 7},
        },
        "cleanup-old-logs": {
            "task": "apps.backend.workers.tasks.cleanup_tasks.cleanup_old_logs",
            "schedule": timedelta(days=1),
            "options": {"queue": "low_priority"},
            "kwargs": {"log_directory": "/var/log/celery", "days_old": 30},
        },
        ...
        "generate-daily-analytics": {
            ...
            "schedule": timedelta(days=1),
            "options": {"queue": "analytics"},
            "kwargs": {"report_type": "daily_usage", "include_tenant_breakdown": True},
        },
        ...
        "generate-weekly-reports": {
            ...
            "schedule": timedelta(days=7),
            "options": {"queue": "analytics"},
            "kwargs": {"include_performance_metrics": True, "include_tenant_analytics": True},
        },
        "export-analytics-data": {
            "task": "apps.backend.workers.tasks.analytics_tasks.export_analytics_data",
            "schedule": timedelta(days=1),
            "options": {"queue": "analytics"},
            "kwargs": {"export_format": "json", "data_type": "daily_summary", "retention_days": 90},
        },
        # ========================================
        ...
        "send-pending-notifications": {
            ...
            "options": {"queue": "email"},
            "kwargs": {"notification_types": ["urgent", "important"]},
        },
        "cleanup-email-logs": {
        ...
        "process-content-generation-queue": {
            ...
            "kwargs": {"max_concurrent_generations": 5},
        },
        "cleanup-generated-content": {
            "task": "apps.backend.workers.tasks.content_tasks.cleanup_generated_content",
            "schedule": timedelta(days=1),
            "options": {"queue": "low_priority"},
            "kwargs": {"cleanup_drafts_older_than_days": 30, "cleanup_temp_files": True},
        },
        # ========================================
        # ROBLOX SYNC TASKS
        # ========================================
        "sync-roblox-environments": {
            ...
            "kwargs": {"sync_type": "incremental", "include_analytics": True},
        },
        "validate-roblox-deployments": {
            "task": "apps.backend.workers.tasks.roblox_tasks.validate_deployments",
            "schedule": timedelta(hours=6),
            "options": {"queue": "roblox"},
            "kwargs": {"check_asset_integrity": True, "verify_api_endpoints": True},
        },
        # ========================================
        ...
        "generate-tenant-usage-reports": {
            ...
            "kwargs": {"report_period": "daily", "include_billing_data": True},
        },
        "cleanup-tenant-data": {
            "task": "apps.backend.workers.tasks.tenant_tasks.cleanup_tenant_data",
            "schedule": timedelta(days=7),
            "options": {"queue": "tenant_operations"},
            "kwargs": {"cleanup_inactive_tenants": True, "retention_days": 365},
        },
        "sync-tenant-billing": {
            "task": "apps.backend.workers.tasks.tenant_tasks.sync_billing_data",
            "schedule": timedelta(hours=12),
            "options": {"queue": "tenant_operations"},
            "kwargs": {"include_usage_metrics": True, "update_stripe_metadata": True},
        },
        # ========================================
        # HEALTH MONITORING TASKS
        # ========================================
        "health-check-integrations": {
            ...
            "kwargs": {"check_redis": True, "check_postgres": True, "check_external_apis": True},
        },
        "monitor-system-resources": {
            "task": "apps.backend.workers.tasks.health_tasks.monitor_system_resources",
            "schedule": timedelta(minutes=10),
            "options": {"queue": "default"},
            "kwargs": {"alert_thresholds": {"memory_usage": 85, "cpu_usage": 90, "disk_usage": 80}},
        },
        "check-worker-health": {
            "task": "apps.backend.workers.tasks.health_tasks.check_worker_health",
            "schedule": timedelta(minutes=2),
            "options": {"queue": "high_priority"},
            "kwargs": {"max_response_time": 30, "alert_on_failure": True},
        },
        # ========================================
        ...
        "update-prometheus-metrics": {
            ...
            "options": {"queue": "default"},
            "kwargs": {"metrics_types": ["worker_status", "queue_lengths", "task_rates"]},
        },
        "rotate-log-files": {
        ...
        "backup-redis-data": (
            {
                ...
                "schedule": timedelta(hours=6),
                "options": {"queue": "low_priority"},
                "kwargs": {"backup_location": "/backups/redis", "retention_days": 7},
            }
            if not settings.ENV_NAME.lower().startswith("dev")
            else None
        ),
    }
...
def get_development_schedule() -> dict[str, Any]:
    ...
    return {
        ...
        "dev-cleanup-temp": {
            ...
            "schedule": timedelta(hours=2),
            "options": {"queue": "low_priority"},
            "kwargs": {"temp_directory": "/tmp/toolboxai-dev"},
        },
    }
