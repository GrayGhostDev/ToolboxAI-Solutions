...
class UserGuidanceSystem:
    ...
    async def provide_guidance(
        ...
        if guidance_data.get("examples"):
            guidance["examples"] = self._personalize_examples(guidance_data["examples"], context)
        ...
        for example in examples:
            ...
            if context.personalization:
                if "{{student_names}}" in example and context.personalization.student_names:
                    ...
                    personalized_example = personalized_example.replace("{{student_names}}", name)
    ...
    async def _generate_encouragement(self, context: ConversationContext) -> list[str]:
        ...
        if completed_stages == 0:
            encouragement.append(
                "Great start! You're taking the first step toward creating amazing educational content!"
            )
        elif completed_stages < total_stages / 2:
            encouragement.append(
                "You're making excellent progress! The foundation you're building will make your content truly special."
            )
        elif completed_stages < total_stages * 0.8:
            encouragement.append(
                "You're almost there! The details you're providing will make this content unforgettable for your students."
            )
        else:
            encouragement.append(
                "Fantastic work! You've created a comprehensive plan that will result in outstanding educational content!"
            )

        # Quality-based encouragement
        if context.personalization and context.personalization.student_names:
            encouragement.append(
                f"Your students {', '.join(context.personalization.student_names[:3])} are going to love this personalized experience!"
            )

        if context.uniqueness and context.uniqueness.creative_twists:
            encouragement.append(
                "The creative elements you're adding will make this content truly unique and memorable!"
            )
    ...
    def _example_matches_content_type(
        ...
        if content_type == ContentType.LESSON:
            ...
            return any(
                keyword in title for keyword in ["simulation", "virtual", "lab", "experiment"]
            )
    ...
    async def validate_content_requirements(
            ...
            for obj in requirements.learning_objectives:
                if len(obj.split()) < 5:  # Simple check for specificity
                    validation_result["warnings"].append(
                        f"Learning objective '{obj}' could be more specific"
                    )
        ...
        if requirements.duration_minutes:
            if requirements.duration_minutes < 5:
                validation_result["warnings"].append(
                    "Very short duration may limit learning effectiveness"
                )
            elif requirements.duration_minutes > 120:
                validation_result["warnings"].append(
                    "Very long duration may require breaks or multiple sessions"
                )
        ...
        if requirements.student_count:
            if requirements.student_count > 50:
                validation_result["warnings"].append(
                    "Large class size may require additional management features"
                )
    ...
    async def suggest_improvements(self, context: ConversationContext) -> list[str]:
        ...
        if not context.personalization:
            suggestions.append(
                "Add personalization data to make content more engaging for your students"
            )
        ...
        if (
            ...
            and context.requirements.engagement_level == EngagementLevel.PASSIVE
        ):
            suggestions.append(
                "Consider increasing engagement level for better student participation"
            )
