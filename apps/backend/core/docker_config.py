...
def get_docker_config() -> DockerServiceConfig:
    ...
    if _docker_config is None:
        ...
        logger.info(
            f"Docker configuration initialized for environment: {_docker_config.environment.value}"
        )
...
def get_docker_service_status() -> dict[str, Any]:
    ...
    return {
        ...
        "services": {
            ...
            "mcp": {"host": config.mcp_host, "port": config.mcp_port, "url": config.get_mcp_url()},
        },
    }
