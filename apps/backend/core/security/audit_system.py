...
class SecurityAuditSystem:
    ...
    async def _detect_threat_indicators(self, event: SecurityEvent) -> List[str]:
        ...
        if event.endpoint:
            if any(pattern in event.endpoint.lower() for pattern in ['../', 'union', 'select', '<script']):
    ...
    def _get_compliance_tags(self, event: SecurityEvent) -> List[str]:
        ...
        # GDPR compliance
        if event.event_type in [SecurityEventType.SENSITIVE_DATA_ACCESS, SecurityEventType.DATA_EXPORT]:
            tags.append("gdpr_relevant")
        
        # SOX compliance
        if event.event_type in [SecurityEventType.DATA_MODIFICATION, SecurityEventType.CONFIGURATION_CHANGE]:
    ...
    def _log_to_application(self, event: SecurityEvent):
        """Log to application logger"""
        level = logging.WARNING if event.severity in [SecurityLevel.HIGH, SecurityLevel.CRITICAL] else logging.INFO
