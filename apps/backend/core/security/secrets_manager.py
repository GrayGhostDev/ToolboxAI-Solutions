...
class SecretsManager:
    ...
    def __init__(self, region_name: str = None, use_cache: bool = True):
        ...
        try:
            ...
            logger.warning(
                f"AWS Secrets Manager not available, falling back to environment variables: {e}"
            )
            self.client = None
    ...
    def get_secret(
        self, secret_name: str, fallback_env: Optional[str] = None
    ...
    def rotate_secret(self, secret_name: str) -> bool:
        ...
        try:
            self.client.rotate_secret(
                SecretId=secret_name, RotationLambdaARN=os.getenv("SECRET_ROTATION_LAMBDA_ARN")
            )
