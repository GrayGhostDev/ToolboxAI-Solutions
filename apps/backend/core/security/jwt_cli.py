...
def generate_secret(args):
    ...
    try:
        ...
        if args.instructions:
            instructions = generator.create_environment_instructions(
                secret, getattr(args, "secret_id", "cli-generated")
            )
            print("\n" + instructions)
        else:
...
def validate_secret(args):
    ...
    try:
        ...
        if not secret:
            ...
            if not secret:
                print(
                    "Error: No secret provided. Use --secret or set JWT_SECRET_KEY environment variable"
                )
                sys.exit(1)
    ...
    try:
        ...
        try:
            ...
            if current_secret == "NOT_SET":
                ...
                print(
                    f"   {'✓' if is_valid else '✗'} Current secret: {'Valid' if is_valid else 'Invalid'}"
                )
...
Examples:
  ...
  %(prog)s status
    ...
    gen_parser.add_argument(
        "--length", type=int, default=64, help="Secret length in characters (default: 64)"
    )
    gen_parser.add_argument("--hex", action="store_true", help="Generate hexadecimal secret")
    gen_parser.add_argument("--base64", action="store_true", help="Generate base64 encoded secret")
    gen_parser.add_argument("--store", action="store_true", help="Store secret securely")
    gen_parser.add_argument(
        "--instructions", action="store_true", help="Show deployment instructions"
    )
    ...
    val_parser.add_argument(
        "--secret", type=str, help="Secret to validate (or use JWT_SECRET_KEY env var)"
    )
    ...
    rotate_parser.add_argument(
        "--force", action="store_true", help="Force rotation even if current secret is valid"
    )
