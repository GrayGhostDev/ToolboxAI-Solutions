...
class JWTSecurityManager:
    ...
    def _generate_and_replace_secret(self) -> str | None:
        ...
        try:
            ...
            logger.warning(
                f"New JWT secret generated. Deployment instructions saved to: {instructions_file}"
            )
    ...
    def rotate_secret(self, force: bool = False) -> tuple[bool, dict[str, Any]]:
        ...
        try:
            ...
            if not force and current_secret:
                ...
                if is_valid:
                    rotation_report["rotation_reason"] = (
                        "Rotation not needed - current secret is valid"
                    )
            ...
            logger.warning(
                f"Update environment variables using instructions in: {instructions_file}"
            )
                ...
                if is_valid:
                    if validation_report["entropy_score"] > 4.0 and len(secret) >= 64:
                        ...
                        status["recommendations"].append(
                            "Consider using a longer, more complex secret"
                        )
                else:
