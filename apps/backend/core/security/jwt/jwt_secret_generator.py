...
class JWTSecretGenerator:
    ...
    def generate_secure_secret(length: int = None) -> str:
        ...
        if length < JWTSecretGenerator.MIN_SECRET_LENGTH:
            raise ValueError(
                f"Secret length must be at least {JWTSecretGenerator.MIN_SECRET_LENGTH} characters"
            )
    ...
    def validate_secret(cls, secret: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        if len(secret) < cls.MIN_SECRET_LENGTH:
            report["issues"].append(
                f"Secret too short (minimum {cls.MIN_SECRET_LENGTH} characters)"
            )
        ...
        if entropy_score < cls.ENTROPY_THRESHOLD:
            report["issues"].append(
                f"Low entropy ({entropy_score:.2f} bits/char, minimum {cls.ENTROPY_THRESHOLD})"
            )
    ...
    def generate_and_store_secret(
        self, description: str = "Auto-generated JWT secret"
    ) -> Tuple[str, str]:
        """
    ...
    def rotate_secret(self, current_secret_id: Optional[str] = None) -> Tuple[str, str]:
        ...
        """
        description = (
            f"Rotated from {current_secret_id}" if current_secret_id else "Secret rotation"
        )
