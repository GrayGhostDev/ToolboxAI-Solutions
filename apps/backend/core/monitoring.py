...
class SentryManager:
    ...
    def initialize(self, dsn: Optional[str] = None, **kwargs) -> bool:
        ...
        try:
            ...
            self.add_breadcrumb(
                message="Sentry initialized successfully", category="sentry", level="info"
            )
...
def sentry_database_monitor(operation: str = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            with sentry_manager.start_span(
                op=op, description=f"Database operation: {func.__name__}"
            ):
                try:
                    result = await func(*args, **kwargs)
...
def sentry_external_api_monitor(service_name: str):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            with sentry_manager.start_span(
                op="http.client", description=f"External API call to {service_name}"
            ):
                try:
