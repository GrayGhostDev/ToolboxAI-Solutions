...
This module contains the original error handling middleware and additional
error handling utilities. Core error types have been moved to error_handler.py
"""
...
class ErrorMetrics:
    ...
    def record_error(self, error: dict[str, Any], category: str):
        ...
        self.error_rates[category] = [dt for dt in self.error_rates[category] if dt > cutoff]
...
class LegacyErrorHandler:
    """Legacy error handler (renamed to avoid conflict with new ErrorHandler)"""
    ...
    def _setup_default_mappers(self):
        ...
        self.register_error_mapper(RequestValidationError, self._handle_request_validation_error)
...
class ErrorHandlingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, debug: bool = False):
        super().__init__(app)
        self.error_handler = LegacyErrorHandler(debug=debug)
