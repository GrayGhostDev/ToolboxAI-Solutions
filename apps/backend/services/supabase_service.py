...
class SupabaseService:
    ...
    async def update_agent_instance(self, agent_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.client.table("agent_instances").update(updates).eq("agent_id", agent_id).execute()
    ...
    async def get_agent_instance(self, agent_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = self.client.table("agent_instances").select("*").eq("agent_id", agent_id).execute()
    ...
    async def get_all_agent_instances(self) -> List[Dict[str, Any]]:
        ...
        try:
            result = self.client.table("agent_instances").select("*").order("created_at", desc=True).execute()
    ...
    async def update_task_execution(self, task_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            result = self.client.table("agent_executions").update(updates).eq("task_id", task_id).execute()
    ...
    async def get_task_execution(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = self.client.table("agent_executions").select("*").eq("task_id", task_id).execute()
    ...
    async def get_recent_system_health(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result = (self.client.table("system_health")
                     .select("*")
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if not self.is_available():
            return {
                "healthy": False,
                "error": "Supabase not configured or available"
            }
