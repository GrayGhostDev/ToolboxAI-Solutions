...
class RojoProjectConfig(BaseModel):
    ...
    glob_ignore_paths: Optional[List[str]] = Field(default_factory=lambda: [
        "**/node_modules",
        "**/.git",
        "**/*.lock"
    ])
        ...
        try:
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    return RojoSyncStatus(
                        connected=False,
                        errors=[f"Rojo API returned status {response.status}"]
                    )
