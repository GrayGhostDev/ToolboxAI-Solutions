...
class InvoiceGenerator:
        ...
        try:
            ...
            if customer.stripe_customer_id:
                stripe_invoice = await self._create_stripe_invoice(
                    customer=customer, invoice=invoice, items=invoice_items
                )
    ...
    async def finalize_invoice(self, invoice_id: int, db: AsyncSession) -> dict[str, Any]:
        """
        ...
        try:
            ...
            invoice_query = (
                ...
                .options(selectinload(Invoice.customer), selectinload(Invoice.invoice_items))
            )
    ...
    async def generate_invoice_pdf(self, invoice_id: int, db: AsyncSession) -> bytes | None:
        """
        ...
        try:
            ...
            customer_style = ParagraphStyle("CustomerStyle", parent=styles["Heading2"], fontSize=14)
            elements.append(Paragraph("Bill To:", customer_style))
            elements.append(Spacer(1, 0.1 * inch))

            customer_info = [invoice.customer.name, invoice.customer.email]
    ...
    async def _create_stripe_invoice(
        self, customer: Customer, invoice: Invoice, items: list[InvoiceItem]
    ) -> dict[str, Any] | None:
    ...
    async def _send_invoice_email(self, invoice: Invoice, customer: Customer) -> None:
        ...
        try:
            ...
            await email_service.send_email(
                ...
                template_context={
                    ...
                    "view_invoice_url": invoice.hosted_invoice_url
                    or f"{settings.FRONTEND_URL}/invoices/{invoice.id}",
                    "items": [
                        {
                            "description": item.description,
    ...
    async def mark_invoice_paid(
        self, invoice_id: int, payment_id: int, db: AsyncSession
    ) -> dict[str, Any]:
        """
