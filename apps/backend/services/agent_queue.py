...
class QueuedTask:
    ...
    def __init__(self, task_id: str, agent_type: str, task_type: str, 
...
class AgentTaskQueue:
    ...
    async def enqueue_task(self, agent_type: str, task_type: str, task_data: Dict[str, Any],
    ...
    async def complete_task(self, task_id: str, result: Dict[str, Any]):
        ...
        await trigger_task_event(
            ...
            task_id,
            "queue",
            {"result": result, "execution_time": (task.completed_at - task.started_at).total_seconds()},
            task.user_id
        )
    ...
    async def fail_task(self, task_id: str, error_message: str):
        ...
        if task.retry_count <= task.max_retries:
            ...
            logger.info(f"Retrying task {task_id} in {retry_delay} seconds (attempt {task.retry_count})")
        else:
            ...
            logger.error(f"Task {task_id} moved to dead letter queue after {task.retry_count} failures")
        ...
        await trigger_task_event(
            ...
            task_id,
            "queue",
            {"error": error_message, "retry_count": task.retry_count, "max_retries": task.max_retries},
            task.user_id
        )
