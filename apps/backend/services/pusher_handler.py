...
class PusherHandler:
        ...
        try:
            ...
            if handler:
                ...
                await self._send_error_message(
                    user_id, f"Unknown message type: {message_type}", channel
                )
    ...
    async def _handle_agent_chat_user(
        ...
        try:
            ...
            result = await roblox_ai_agent.handle_user_message(conversation_id, message, context)
    ...
    async def _handle_ai_message(
        ...
        try:
            ...
            # Process message with AI agent
            response = await roblox_ai_agent.handle_user_message(conversation_id, message, context)
    ...
    async def _handle_roblox_agent_request(
        ...
        try:
            ...
            await self._send_to_channel(
                ...
                {"conversationId": conversation_id, "spec": spec, "status": "started"},
            )

            # Monitor progress and send updates
            asyncio.create_task(self._monitor_environment_generation(conversation_id, user_id))
    ...
    async def _handle_analytics_request(
        ...
        try:
            ...
            await self._send_to_channel(
                self.channels["analytics"], "analytics_update", analytics_data
            )
    ...
    async def _handle_design_file_process(
        ...
        try:
            ...
            await self._send_error_message(
                user_id, f"Design file processing error: {str(e)}", channel
            )
    ...
    async def _handle_design_file_search(
        ...
        try:
            ...
            await self._send_error_message(user_id, f"Design file search error: {str(e)}", channel)

    async def _monitor_environment_generation(self, conversation_id: str, user_id: str) -> None:
    ...
    async def _send_to_user(
        ...
        try:
            ...
            pusher_trigger_event(user_channel, "message", message)

            # Also send to specified channel if provided
            if channel:
                pusher_trigger_event(channel, "message", message)
    ...
    async def _send_error_message(
        self, user_id: str, error_message: str, channel: str = None
    ) -> None:
    ...
    def subscribe_user_to_channel(self, user_id: str, channel: str) -> bool:
    ...
    def unsubscribe_user_from_channel(self, user_id: str, channel: str) -> bool:
