...
class SemanticCacheService:
    ...
    def __init__(self):
        self.enabled = (
            LANGCACHE_AVAILABLE and os.getenv("LANGCACHE_ENABLED", "false").lower() == "true"
        )
        ...
        try:
            async with self.get_langcache() as lang_cache:
                ...
                search_response = lang_cache.search(
                    prompt=cache_key, similarity_threshold=self.similarity_threshold
                )
    ...
    async def clear_cache(self, model: str | None = None) -> bool:
        """
        ...
        try:
            ...
            if model:
                keys_to_remove = [
                    k for k in self.fallback_cache.keys() if k.startswith(f"{model}:")
                ]
    ...
    def _track_cost_savings(self, model: str | None = None):
    ...
    async def warm_cache(self, common_prompts: list[tuple[str, str, str | None]]):
        """
