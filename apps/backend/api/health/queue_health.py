...
async def get_queue_health():
    ...
    try:
        ...
        performance_metrics = {
            ...
            "memory_efficiency": (
                "good" if redis_status.get("memory_usage", 0) < 1024 * 1024 * 100 else "high"
            ),  # 100MB threshold
        }
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=f"Queue unhealthy: {e}"
        )
        ...
        try:
            ...
            for queue_name in queue_names:
                try:
                    ...
                    queue_details[queue_name] = {"length": 0, "status": "not_found"}
    ...
    try:
        ...
        performance_metrics = {
            ...
            "queue_performance": {
                ...
                "performance_grade": (
                    "excellent" if response_time < 50 else "good" if response_time < 200 else "poor"
                ),
...
async def get_queue_tasks_summary():
    ...
    try:
        ...
        return {
            ...
            "queue_health": (
                "healthy"
                if task_summary["failed_tasks"] < task_summary["total_tasks"] * 0.1
                else "degraded"
            ),
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
