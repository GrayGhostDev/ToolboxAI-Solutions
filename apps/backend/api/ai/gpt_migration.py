...
# Metrics
gpt_api_requests = Counter('gpt41_api_requests_total', 'Total GPT-4.1 API requests', ['model', 'status'])
...
class ModelSelector:
    ...
    def analyze_complexity(messages: List[Dict[str, str]]) -> float:
        ...
        factors = {
            ...
            'code': 0.3 if any(kw in last_message.lower() for kw in ['code', 'function', 'class', 'def', 'implement']) else 0,
            'reasoning': 0.4 if any(kw in last_message.lower() for kw in ['explain', 'why', 'how', 'analyze', 'compare']) else 0,
            'math': 0.3 if any(kw in last_message.lower() for kw in ['calculate', 'solve', 'equation', 'formula']) else 0,
            'creative': 0.3 if any(kw in last_message.lower() for kw in ['create', 'generate', 'write', 'design']) else 0,
            'multi_step': 0.2 if any(kw in last_message.lower() for kw in ['step', 'first', 'then', 'finally']) else 0,
        }
...
class GPT41MigrationClient:
        ...
        **kwargs
        ...
        try:
            ...
            if 'usage' in response_dict:
                ...
                gpt_api_tokens.labels(model=model.value, type='input').inc(usage.get('prompt_tokens', 0))
                gpt_api_tokens.labels(model=model.value, type='output').inc(usage.get('completion_tokens', 0))
    ...
    async def chat_completion(
        ...
        **kwargs
        ...
        if self.config.enable_prompt_optimization and model in [GPTModel.GPT_4_1, GPTModel.GPT_4_1_MINI, GPTModel.GPT_4_1_NANO]:
            optimized_messages = []
            for msg in messages:
                if msg.get('role') == 'user':
                    optimization = self.prompt_optimizer.optimize_for_gpt41(
                        msg['content'],
                        model
                    )
