...
from apps.backend.models.schemas import Session

from .auth import AuthenticationError, JWTManager, User
...
class FastAPIWebSocketAuthenticator:
    ...
    async def authenticate_connection(
        ...
        try:
            ...
            await session.send_auth_message(
                "auth_success",
                {
            ...
            logger.info(
                f"WebSocket authentication successful for user {user_id} (session {session.session_id})"
            )
            return session
    ...
    async def refresh_session_token(self, session_id: str, new_token: str) -> bool:
        """
        ...
        try:
            ...
            if new_user_id != session.user.id:
                logger.warning(
                    f"User ID mismatch during token refresh: {session.user.id} vs {new_user_id}"
                )
            ...
            await session.send_auth_message(
                "token_refreshed", {"success": True, "expires_at": payload.get("exp")}
            )
...
async def authenticate_websocket_message(
    session: WebSocketAuthSession, message: Dict[str, Any]
) -> bool:
    """
