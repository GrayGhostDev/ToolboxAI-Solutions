...
async def trigger_realtime_event(
    ...
    try:
        ...
        if not channel or not event:
            raise HTTPException(
                status_code=400,
                detail="Missing required fields: channel, event"
            )
...
async def get_pusher_stats(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve Pusher statistics: {str(e)}"
        )
...
def _can_access_channel(user: User, channel_name: str) -> bool:
    ...
    if user_role == 'teacher' and (
        channel_name.startswith("private-student-") or
        channel_name.startswith("private-class-")
    ):
        return True
...
def _can_trigger_event(user: User, channel: str, event: str) -> bool:
    ...
    if user_role == 'teacher' and (
        event.startswith("content-") or
        event.startswith("lesson-") or
        event.startswith("assessment-")
    ):
        return True
