...
async def create_lesson(
    ...
    new_lesson = {
        **lesson_data,  # Include all submitted data
        ...
        "teacher_id": (
            current_user.id if role == "teacher" else lesson_data.get("teacher_id", current_user.id)
        ),
        "teacher_name": (
    ...
    logger.info(
        f"Created mock lesson: {new_lesson.get('title', 'Unnamed')} for user {current_user.email}"
    )
    return new_lesson
