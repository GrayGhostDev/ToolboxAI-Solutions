    ...
    **Response:**
    ...
    try:
        ...
        if not verify_password(
            password_request.current_password, user_data.get("password_hash", "")
        ):
        ...
        async with get_db_session() as db:
            await update_user_password(
                db=db, user_id=current_user.id, password_hash=result.get("password_hash")
            )
...
async def reset_user_password(
    ...
    **Response:**
    ...
    if current_user.role != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Only administrators can reset passwords"
        )
    ...
    try:
        ...
        async with get_db_session() as db:
            await update_user_password(
                db=db, user_id=reset_request.user_id, password_hash=result["password_hash"]
            )
    ...
    **Response:**
    ...
    try:
        count = session_manager.invalidate_all_user_sessions(
            user_id=current_user.id, reason="manual_logout_all"
        )
