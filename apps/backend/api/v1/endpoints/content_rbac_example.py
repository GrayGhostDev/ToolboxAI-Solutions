...
from database.models import EducationalContent, User
...
async def list_content(
    ...
    contents = (
        ...
        .filter(EducationalContent.organization_id == org_id)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
async def create_content(
    ...
    content = EducationalContent(
        **content_data.dict(), created_by=user.id, organization_id=org_id, status="draft"
    )
...
async def get_content(
    content_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_session)
):
    """
...
@require_permissions(["content:publish:organization", "content:publish:all"], require_all=False)
...
@require_permissions(["content:read:organization", "content:create:organization"], require_all=True)
...
async def get_organization_content_stats(
    ...
    """
    total_content = (
        db.query(EducationalContent).filter(EducationalContent.organization_id == org_id).count()
    )

    published_content = (
        db.query(EducationalContent)
        .filter(
            EducationalContent.organization_id == org_id, EducationalContent.status == "published"
        )
        .count()
    )
    ...
    draft_content = (
        db.query(EducationalContent)
        .filter(EducationalContent.organization_id == org_id, EducationalContent.status == "draft")
        .count()
    )
