...
class LearningPreferences(BaseModel):
    ...
    session_duration_preference: int = Field(30, ge=5, le=120, description="Preferred session length in minutes")
...
def calculate_profile_completion(user: UserProfile) -> int:
    ...
    required_fields = [
        ...
        user.learning_preferences, user.notification_settings
    ]
...
async def create_user(
    ...
    try:
        ...
        existing_user = next(
            (u for u in _mock_users_db.values() if u.email == request.email or u.username == request.username),
            None
        )
        if existing_user:
            raise HTTPException(
        ...
        user_profile = UserProfile(
            ...
            learning_preferences=request.learning_preferences or LearningPreferences(
        ...
        if request.role == UserRole.STUDENT and request.parent_email:
            # Find parent user
            parent_user = next(
                (u for u in _mock_users_db.values() if u.email == request.parent_email),
                None
            )
            if parent_user:
...
async def list_users(
    ...
    try:
        ...
        if user_role == "teacher":
            ...
            users = [u for u in users if u.role == UserRole.STUDENT and u.school_id == current_user.get("school_id")]
...
async def update_user(
    ...
    try:
        ...
        if current_user_id != user_id and current_user_role not in ["admin", "district_admin", "school_admin"]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
@router.post("/users/{user_id}/achievements", response_model=Achievement, status_code=status.HTTP_201_CREATED)
async def award_achievement(
    ...
    try:
        ...
        if user_id in _mock_progress_db:
            ...
            _mock_progress_db[user_id].recent_achievements = _mock_progress_db[user_id].recent_achievements[:10]
        ...
        logger.info(f"Achievement awarded: {achievement_id} to user {user_id} by {current_user['id']}")
        return achievement
...
async def get_parent_dashboard(
    current_user: Dict = Depends(get_current_user),
    _: None = Depends(require_role("parent"))
):
    """
    ...
    try:
        ...
        children = [_mock_users_db[child_id] for child_id in child_ids if child_id in _mock_users_db]
        ...
        combined_progress = {
            ...
            "total_lessons_completed": sum(
                _mock_progress_db.get(child.id, UserProgress(
                    user_id=child.id, total_lessons_completed=0, total_time_spent=0,
                    current_streak=0, longest_streak=0, total_points=0, level=1,
                    subject_progress={}, recent_achievements=[], weekly_activity=[], competency_levels={}
                )).total_lessons_completed for child in children
            ),
            "average_time_per_week": sum(
                _mock_progress_db.get(child.id, UserProgress(
                    ...
                    current_streak=0, longest_streak=0, total_points=0, level=1,
                    subject_progress={}, recent_achievements=[], weekly_activity=[], competency_levels={}
                )).total_time_spent for child in children
            ) / max(len(children), 1) / 4  # Assuming 4 weeks average
        }
        ...
        dashboard = ParentDashboard(
            ...
            achievement_summary={
                "total_achievements": sum(len(_mock_achievements_db.get(child.id, [])) for child in children),
                "points_this_week": sum(
                    _mock_progress_db.get(child.id, UserProgress(
                        user_id=child.id, total_lessons_completed=0, total_time_spent=0,
                        current_streak=0, longest_streak=0, total_points=0, level=1,
                        subject_progress={}, recent_achievements=[], weekly_activity=[], competency_levels={}
                    )).total_points for child in children
...
async def link_parent_to_student(
    ...
    try:
        ...
        student = next(
            (u for u in _mock_users_db.values() if u.email == request.student_email),
            None
        )
        parent = next(
            (u for u in _mock_users_db.values() if u.email == request.parent_email),
        ...
        if parent.role != UserRole.PARENT:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Target user is not a parent"
            )
