    ...
    try:
        ...
        if customer.user:
            await email_queue.enqueue(
                ...
                template_data={
                    "customer_name": customer.user.name,
                    "plan_name": subscription["items"]["data"][0]["price"].get("nickname", "Subscription"),
...
async def handle_invoice_paid(
    ...
    try:
        ...
        if existing_invoice:
            ...
            existing_invoice.paid_at = datetime.fromtimestamp(invoice["status_transitions"]["paid_at"])
        else:
            new_invoice = Invoice(
        ...
        if subscription:
            await dunning_service.clear_dunning_state(
    ...
    try:
        ...
        payment = await db.scalar(
            select(Payment).where(
                Payment.stripe_payment_intent_id == charge.get("payment_intent")
            )
        )
