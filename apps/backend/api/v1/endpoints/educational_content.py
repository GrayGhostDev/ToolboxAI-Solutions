...
async def create_content(
    ...
    try:
        ...
        background_tasks.add_task(notify_content_update, content_id, "created", current_user["id"])
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create content"
        )
...
async def list_content(
    ...
    try:
        ...
        if difficulty_level:
            filtered_content = [
                c for c in filtered_content if c.difficulty_level == difficulty_level
            ]
        if status:
        ...
        summaries = [
            ContentSummary(
                ...
                usage_count=_mock_analytics_db.get(
                    content.id,
                    ContentAnalytics(
...
async def get_content(content_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        if user_role == "student" and content.status != ContentStatus.PUBLISHED:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Content not available to students"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve content"
        )
...
async def update_content(
    ...
    try:
        ...
        if current_user.get("role") == "teacher" and content.created_by != current_user["id"]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="You can only edit your own content"
            )
        ...
        # Background notification
        background_tasks.add_task(notify_content_update, content_id, "updated", current_user["id"])
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update content"
        )
...
async def delete_content(
    ...
    try:
        ...
        if current_user.get("role") == "teacher" and content.created_by != current_user["id"]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="You can only delete your own content"
            )
        ...
        # Background notification
        background_tasks.add_task(notify_content_update, content_id, "deleted", current_user["id"])
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to delete content"
        )


@router.post(
    "/generate", response_model=ContentGenerationResponse, status_code=status.HTTP_202_ACCEPTED
)
...
async def publish_content(
    ...
    try:
        ...
        background_tasks.add_task(
            notify_content_update, content_id, "published", current_user["id"]
        )
...
async def search_curriculum_standards(
    ...
    try:
        ...
        filtered_standards = [
            standard
            for standard in mock_standards
            if query_lower in standard.description.lower()
            or query_lower in standard.standard_id.lower()
        ]
