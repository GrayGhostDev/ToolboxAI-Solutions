...
async def bulk_upload_files(
    ...
    try:
        ...
        successful_file_ids = [
            str(r.file_id) for r in file_results
            if r.status == "success" and r.file_id
        ]
...
async def bulk_delete_files(
    ...
    background_tasks: BackgroundTasks,
    storage_service: StorageService = Depends(get_storage_service),
    user_tenant: tuple[User, TenantContext] = Depends(require_tenant_manager)  # Requires manager role
):
    """
    ...
    try:
        ...
        for file_id in delete_request.file_ids:
            try:
                ...
                if not file_info:
                    file_results.append(BulkDeleteFileResult(
                        file_id=file_id,
                        filename="unknown",
                        status="not_found",
    ...
    try:
        ...
        for file_id in share_request.file_ids:
            try:
                ...
                if not file_info:
                    file_results.append(BulkShareFileResult(
                        file_id=file_id,
                        filename="unknown",
                        status="not_found",
                        error_message="File not found"
