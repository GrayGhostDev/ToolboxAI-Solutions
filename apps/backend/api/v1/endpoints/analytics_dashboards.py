...
async def get_dashboard_data(
    ...
    """
    try:
        logger.info(f"Getting data for dashboard {dashboard_id}, refresh={refresh}")
...
async def clone_dashboard(
    ...
    """
    try:
        logger.info(f"User {current_user.id} cloning dashboard {dashboard_id} as '{new_name}'")
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Source dashboard not found"
        )
