...
from celery import Task, shared_task, group, chain, chord, signature
from celery.exceptions import Retry, SoftTimeLimitExceeded, MaxRetriesExceededError
...
@shared_task(
    bind=True,
    name='tasks.cleanup.temp_storage',
    queue='low_priority'
)
def cleanup_temp_storage(self) -> Dict[str, Any]:
    ...
    for temp_dir in temp_dirs:
        result = cleanup_old_files.apply_async(
            args=[temp_dir, 1],  # Clean files older than 1 day
            queue='low_priority'
        ).get(timeout=60)
...
def generate_educational_content(
    ...
    try:
        ...
        return {
            'status': 'timeout',
            'error': 'Content generation took too long'
        }
    except Exception as exc:
...
@shared_task(
    bind=True,
    name='tasks.content.generate_quiz',
    queue='default'
)
...
@shared_task(
    bind=True,
    name='tasks.content.analyze_quality',
    queue='low_priority'
)
...
@shared_task(
    bind=True,
    name='tasks.notifications.send_bulk',
    queue='low_priority'
)
def send_bulk_notifications(
    ...
    try:
        ...
        job = group(
            send_notification.s(
                user_id, notification_type, title, message, None, ['pusher']
            )
...
@shared_task(
    bind=True,
    name='tasks.analytics.generate_report',
    queue='low_priority'
)
...
@shared_task(
    bind=True,
    name='tasks.analytics.export_data',
    queue='low_priority'
)
...
@shared_task(
    bind=True,
    name='tasks.roblox.sync_environment',
    queue='default'
)
...
@shared_task(
    bind=True,
    name='tasks.roblox.validate_assets',
    queue='low_priority'
)
