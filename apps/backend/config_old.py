...
class Settings(BaseSettings):
    ...
    ROBLOX_PLUGIN_PORT: int = Field(
        default=64989, alias="ROBLOX_PLUGIN_PORT"
    )
    ...
    OPENAI_TEMPERATURE: float = Field(
        default=0.7, alias="OPENAI_TEMPERATURE"
    )
    ...
    RATE_LIMIT_PER_MINUTE: int = Field(
        default=100, alias="RATE_LIMIT_PER_MINUTE"
    )
    ...
    BYPASS_RATE_LIMIT_IN_TESTS: bool = Field(
        default=True, alias="BYPASS_RATE_LIMIT_IN_TESTS"
    )
    ...
    MAX_CONTENT_GENERATION_TIME: int = Field(
        default=300, alias="MAX_CONTENT_GENERATION_TIME"
    )
    MAX_CONCURRENT_GENERATIONS: int = Field(
        default=10, alias="MAX_CONCURRENT_GENERATIONS"
    )
    ...
    WS_HEARTBEAT_INTERVAL: int = Field(
        default=30, alias="WS_HEARTBEAT_INTERVAL"
    )
    WS_RATE_LIMIT_PER_MINUTE: int = Field(
        default=100, alias="WS_RATE_LIMIT_PER_MINUTE"
    )
    ...
    WS_CHANNEL_ROLE_PREFIXES: Dict[str, str] = Field(
        default={"admin_": "admin", "teacher_": "teacher"},
    ...
    MCP_MAX_CONTEXT_TOKENS: int = Field(
        default=128000, alias="MCP_MAX_CONTEXT_TOKENS"
    )
    ...
    def get_sentry_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "traces_sample_rate": (
                0.1 if self.ENVIRONMENT == "production" 
                else self.SENTRY_TRACES_SAMPLE_RATE
            ),
            "profiles_sample_rate": (
                0.1 if self.ENVIRONMENT == "production" 
                else self.SENTRY_PROFILES_SAMPLE_RATE
            ),
            "send_default_pii": (
                False if self.ENVIRONMENT == "production" 
                else self.SENTRY_SEND_DEFAULT_PII
            ),
