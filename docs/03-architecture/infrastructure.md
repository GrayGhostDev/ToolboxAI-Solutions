...
# DEPRECATED: WebSocket upstream (replaced by Pusher Channels)
...
# Real-time communication now handled by Pusher Channels
# No dedicated WebSocket backend servers required
...
server {
    ...
    # DEPRECATED: WebSocket endpoints (replaced by Pusher Channels)
    # Real-time communication now handled by Pusher Channels service
    # WebSocket endpoints maintained for backward compatibility only
    location /ws/ {
        # Return 410 Gone to indicate deprecated endpoint
        return 410 '{"error":"WebSocket endpoints deprecated","migration_guide":"/docs/WEBSOCKET_TO_PUSHER_MIGRATION_COMPLETE.md"}';
        add_header Content-Type application/json;
        add_header Deprecation "true";
        add_header Sunset "2025-06-01";
        add_header Link '</docs/WEBSOCKET_TO_PUSHER_MIGRATION_COMPLETE.md>; rel="deprecation"';
    }

    # Pusher webhook endpoints
    location /api/v1/pusher/ {
        proxy_pass http://api_backend/api/v1/pusher/;
        proxy_http_version 1.1;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        ...
        proxy_set_header X-Pusher-Key $http_x_pusher_key;
        proxy_set_header X-Pusher-Signature $http_x_pusher_signature;
    }
...
- Cache hit rates
- ~~WebSocket connections~~ **DEPRECATED**: Replaced by Pusher Channels
