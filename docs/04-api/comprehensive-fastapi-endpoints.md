...
- Real-time collaboration via Pusher Channels (migrated from WebSocket)
...
- ~~`WebSocket /api/v1/roblox-integration/environments/{environment_id}/realtime`~~ - **DEPRECATED**: Use Pusher channel `roblox-environment-{environment_id}` instead
...
- ~~`WebSocket /api/v1/ai-agents/realtime/{connection_type}`~~ - **DEPRECATED**: Use Pusher channel `agent-status` instead
...
Integrated with Pusher Channels for scalable real-time communication:
```python
# NEW - Pusher integration (current)
from apps.backend.services.pusher import trigger_event

# OLD - WebSocket manager (deprecated)
...
- ~~WebSocket connections~~ **DEPRECATED**: Use Pusher Channels for live updates
