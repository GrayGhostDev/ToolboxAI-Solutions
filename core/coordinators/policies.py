...
def check_no_stubs(diff_text: str) -> Tuple[bool, str]:
    ...
    return (m is None, "No stubs/simplifications found" if m is None else f"Found stub-like marker: {m.group(0)}")


def check_tests() -> Tuple[bool, str]:
    r = _run(["pytest","-q","--maxfail=1","--disable-warnings","--json-report","--json-report-file=reports/pytest.json","--cov","--cov-report=xml:reports/coverage.xml"])
    if r.returncode != 0:
