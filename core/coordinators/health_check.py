...
class AgentCoordinatorHealthCheck:
    ...
    async def _check_coordinator_service(self) -> dict[str, Any]:
        ...
        try:
            if self.coordinator:
                is_running = (
                    hasattr(self.coordinator, "is_running") and self.coordinator.is_running()
                )
                ...
                return {
                    ...
                    "details": {
                        ...
                        "coordinator_type": (
                            type(self.coordinator).__name__ if self.coordinator else "Unknown"
                        ),
                    },
                }
            else:
        ...
        try:
            ...
            if self.coordinator and hasattr(self.coordinator, "agents"):
                ...
                for agent_id, agent in agents.items():
                    ...
                    agent_stats["agent_types"][agent_type] = (
                        agent_stats["agent_types"].get(agent_type, 0) + 1
                    )
            ...
            return {"healthy": agent_stats["error_agents"] == 0, "details": agent_stats}
        except Exception as e:
            return {"healthy": False, "error": str(e)}
        ...
        try:
            ...
            if self.coordinator:
                ...
                if hasattr(self.coordinator, "communication_channels"):
                    communication_stats["communication_channels"] = len(
                        self.coordinator.communication_channels
                    )
            ...
            return {
                "healthy": communication_stats["message_queue_size"]
                < 1000,  # Alert if queue is too large
    ...
    async def _check_swarm_coordination(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"healthy": swarm_stats["swarm_health"], "details": swarm_stats}
        except Exception as e:
            return {"healthy": False, "error": str(e)}
    ...
    async def _check_task_queue(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"healthy": task_stats["queue_health"], "details": task_stats}
        except Exception as e:
            return {"healthy": False, "error": str(e)}
    ...
    async def _check_system_resources(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "healthy": (
                    cpu_percent < 80
                    and memory_percent < 80
                    and system_memory.percent < 90
...
class AgentCoordinatorHealthServer:
    ...
    async def metrics_handler(self, request):
        ...
        try:
            ...
            metrics.append(
                f'agent_coordinator_uptime_seconds {{service="agent-coordinator"}} {health_data.get("uptime_seconds", 0)}'
            )
            ...
            if "active_agents" in health_data.get("checks", {}):
                ...
                metrics.append(
                    f'agent_coordinator_total_agents {{service="agent-coordinator"}} {agent_details.get("total_agents", 0)}'
                )
                metrics.append(
                    f'agent_coordinator_healthy_agents {{service="agent-coordinator"}} {agent_details.get("healthy_agents", 0)}'
                )
                metrics.append(
                    f'agent_coordinator_error_agents {{service="agent-coordinator"}} {agent_details.get("error_agents", 0)}'
                )
            ...
            if "task_queue" in health_data.get("checks", {}):
                ...
                metrics.append(
                    f'agent_coordinator_pending_tasks {{service="agent-coordinator"}} {task_details.get("pending_tasks", 0)}'
                )
                metrics.append(
                    f'agent_coordinator_completed_tasks {{service="agent-coordinator"}} {task_details.get("completed_tasks", 0)}'
                )
                metrics.append(
                    f'agent_coordinator_failed_tasks {{service="agent-coordinator"}} {task_details.get("failed_tasks", 0)}'
                )
            ...
            if "system_resources" in health_data.get("checks", {}):
                ...
                metrics.append(
                    f'agent_coordinator_cpu_percent {{service="agent-coordinator"}} {resource_details.get("process_cpu_percent", 0)}'
                )
                metrics.append(
                    f'agent_coordinator_memory_percent {{service="agent-coordinator"}} {resource_details.get("process_memory_percent", 0)}'
                )
