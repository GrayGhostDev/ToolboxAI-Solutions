...
class TaskRegistry:
    ...
    def k_task(self, tid): return self._k("task", tid)
    def k_state(self, state): return self._k("tasks", "state", state)
    def k_lock(self, tid): return self._k("task", "lock", tid)
    def k_claim(self, tid): return self._k("task", "claim", tid)
    def k_worker_hb(self, wid): return self._k("worker", "hb", wid)

    def enqueue(self, payload: Dict[str, Any], priority: float = 1.0, idempotency_key: Optional[str] = None) -> str:
    ...
    def claim(self, worker_id: str, capability: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        tids = [tid for tid, _ in self.r.r.zrange(self.k_state(TaskState.QUEUED.value), 0, 49, withscores=True)]
        for tid in tids:
            raw = self.r.r.get(self.k_task(tid)); task = json.loads(raw) if raw else None
            if not task:
                self.r.r.zrem(self.k_state(TaskState.QUEUED.value), tid)
                continue
            if capability and task["payload"].get("capability") not in [capability, "*", None]:
                continue
    ...
    def start(self, tid: str):
        raw = self.r.r.get(self.k_task(tid)); t = json.loads(raw) if raw else None
        if not t: return
    ...
    def complete(self, tid: str):
        ...
        pipe.delete(self.k_lock(tid)); pipe.delete(self.k_claim(tid))
