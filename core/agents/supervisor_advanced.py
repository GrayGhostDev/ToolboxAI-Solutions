...
import time
import traceback
from typing import Dict, Any, List, Optional, Literal, Tuple, Set, Union, Callable
...
try:
    from core.database.connection_manager import get_async_session, get_redis_client, db_manager
    from core.database.models import (
        User, Course, Lesson, Content, Quiz, Analytics, 
        UserProgress, Achievement, ContentStatus
    )
...
try:
    from core.sparc.state_manager import StateManager, EnvironmentState, StateType
...
class AdvancedSupervisorAgent:
    ...
    def __init__(self, config: Optional[AgentConfig] = None):
        ...
        self.redis_client = get_redis_client() if DATABASE_AVAILABLE else None
    ...
    def _build_workflow_graph(self) -> None:
        ...
        workflow.add_conditional_edges(
            ...
            {
                "requires_approval": "wait_approval",
                "no_approval": "route_agents"
            }
        )
    ...
    async def execute_workflow(
        ...
        try:
            ...
            execution.status = WorkflowStatus.COMPLETED if not final_state.error else WorkflowStatus.FAILED
            execution.completed_at = datetime.now()
            execution.result = final_state.result
            execution.error = final_state.error
            execution.metrics = final_state.performance_metrics
    ...
    async def _analyze_task(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        try:
            ...
            try:
                ...
                analysis = {"analysis": response.content, "structured": False}
            
            state.metadata["task_analysis"] = analysis
            state.messages.append(AIMessage(content=f"Task analysis completed: {response.content}"))
            
            # Update performance metrics
            state.performance_metrics["total_tokens"] += response.usage.total_tokens if hasattr(response, 'usage') else 0
    ...
    async def _execute_parallel_agents(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        try:
            ...
            state.metadata["parallel_results"] = {
                ...
                "success_rate": len(successful_results) / len(selected_agents) if selected_agents else 0
            }
    ...
    async def _monitor_agent_execution(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        if parallel_results:
            total_agents = len(parallel_results.get("successful", [])) + len(parallel_results.get("failed", []))
        ...
        logger.info(f"Execution monitoring - Success rate: {success_rate:.2f} ({successful_agents}/{total_agents})")
    ...
    async def _aggregate_results(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        try:
            ...
            try:
                aggregated_result = json.loads(response.content)
            except json.JSONDecodeError:
                aggregated_result = {"content": response.content, "structured": False}
    ...
    async def _validate_result_quality(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        try:
            ...
            try:
                ...
                quality_assessment = {"feedback": response.content}
            ...
            logger.info(f"Quality validation - Score: {quality_score:.2f}, Status: {quality_status}")
    ...
    async def _finalize_workflow(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        # Final logging
        logger.info(f"Workflow {state.execution_id} finalized - Status: {state.status}, Duration: {execution_time:.2f}s")
    ...
    async def _create_agent(self, agent_type: str) -> Optional[BaseAgent]:
        ...
        try:
            ...
            module = import_module(module_name, package="agents")
    ...
    async def _record_agent_success(self, agent_type: str, result: TaskResult, execution_time: float = 0.0):
        ...
        if execution_time > 0:
            ...
            health.average_response_time = (current_avg * (health.total_requests - 1) + execution_time) / health.total_requests
    ...
    async def _record_agent_failure(self, agent_type: str, error: str, execution_time: float = 0.0):
        ...
        # Update success rate
        success_count = health.total_requests - health.error_count
        health.success_rate = success_count / health.total_requests if health.total_requests > 0 else 0.0
        ...
        while True:
            try:
                for agent_id, health in self.agent_health.items():
                    ...
                    if health.last_success and (datetime.now() - health.last_success).total_seconds() > 3600:
            ...
            try:
                ...
                if self.redis_client:
                    ...
                    await self.redis_client.hset(
                        ...
                        mapping={
                            "timestamp": datetime.now().isoformat(),
                            "metrics": json.dumps(self.performance_metrics)
                        }
                    )
