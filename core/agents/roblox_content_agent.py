...
class RobloxScriptResult(BaseModel):
    ...
    learning_integration: dict[str, Any] = Field(
        description="How this integrates with learning objectives"
    )
    estimated_complexity: str = Field(
        description="Complexity level: beginner, intermediate, advanced"
    )
    dependencies: list[str] = Field(
        default_factory=list, description="Required dependencies or assets"
    )
    validation_tests: list[str] = Field(
        default_factory=list, description="Suggested tests to validate the script"
    )
...
Always provide complete, working code with explanations."""
    ...
    async def _generate_roblox_content(
        self, spec: RobloxContentSpec, policy_decision: dict[str, Any]
    ) -> RobloxScriptResult:
...
Ensure the code is production-ready and educationally valuable."""
    ...
    async def _parse_generation_response(
        self, response: str, spec: RobloxContentSpec
    ) -> RobloxScriptResult:
    ...
    async def _enhance_and_validate_result(
        self, result: RobloxScriptResult, spec: RobloxContentSpec
    ) -> RobloxScriptResult:
        ...
        await self._check_roblox_services(result.lua_code)
    ...
    async def generate_educational_script(
        ...
        return await self.execute(
            f"Generate educational {script_type} script for {subject}", context
        )

    async def create_learning_environment(
        self, theme: str, grade_level: int, interactive_elements: list[str]
    ) -> TaskResult:
            ...
            await websocket.send_json(
                {
                    ...
                    "session_id": session_id,
                    "result": (
                        result.output if isinstance(result.output, dict) else result.output.__dict__
                    ),
