...
class CostTracker:
    ...
    def track_request_cost(
        ...
        logger.debug(
            f"Tracked cost: ${cost:.4f} for {model} ({input_tokens + output_tokens} tokens)"
        )
    ...
    def project_monthly_cost(self) -> dict[str, Any]:
        ...
        current_month_analysis = self.get_cost_analysis(start_date=month_start, end_date=now)
    ...
    def set_budget_limit(self, period: str, limit: float, alert_threshold: float = 0.8) -> None:
        ...
        logger.info(
            f"Set {period} budget limit to ${limit:.2f} (alert at {alert_threshold*100:.1f}%)"
        )
                ...
                efficiency_ranking.append(
                    {
                        ...
                        / cost_per_request,  # Higher is better
                    }
                )
