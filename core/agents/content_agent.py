    ...
    def __init__(self, config: Optional[AgentConfig] = None):
        ...
        if config is None:
            ...
            if not self.env_config.use_mock_llm and os.getenv("OPENAI_API_KEY"):
                ...
                model = (
                    "gpt-4o"
                    if os.getenv("USE_GPT4", "false").lower() == "true"
                    else "gpt-3.5-turbo"
                )
            else:
                model = "gpt-3.5-turbo"
        ...
        logger.info(
            f"ContentAgent initialized - Database: {self.database_available}, SPARC: {self.sparc_available}, Real Data: {self.use_real_data}"
        )
...
- Provide detailed metadata for content management systems"""
    ...
    def _init_sparc_components(self):
        """Initialize SPARC framework components"""
        try:
            self.state_manager = create_state_manager(history_size=100, compression_threshold=50)
            self.policy_engine = create_policy_engine(learning_rate=0.01, exploration_rate=0.1)
            self.action_executor = create_action_executor(max_parallel=3, timeout=30.0)
            ...
            self.context_tracker = create_context_tracker(window_size=20, session_timeout=1800.0)
    ...
    async def _generate_educational_content(
        ...
        if DATABASE_AVAILABLE and self.agent_db:
            try:
                ...
                content_items = (
                    await self.agent_db.get_educational_content() if self.agent_db else []
                )
                ...
                for item in content_items:
                    ...
                    if (
                        topic.lower() in item_title
                        or topic.lower() in item_desc
                        or subject.lower() in item_subject
                    ):
                        if item.get("content_data"):
                            ...
                            try:
                                ...
                                logger.info(
                                    f"Using existing content from database: {item['title']}"
                                )
                                break
                            except json.JSONDecodeError:
                                logger.warning(
                                    f"Failed to parse content data for item {item['id']}"
                                )
    ...
    async def _generate_assessments(
        ...
        if DATABASE_AVAILABLE and self.agent_db:
            try:
                ...
                quiz_items = (
                    await self.agent_db.get_quiz_questions(subject=subject) if self.agent_db else []
                )
...
Format for Roblox quiz implementation."""
        ...
        if DATABASE_AVAILABLE and self.agent_db:
            try:
                ...
                (
                    await self.agent_db.save_generated_content("quiz", content_data)
                    if self.agent_db
                    else None
                )
                    ...
                    for std in standards[:5]:  # Limit to 5 standards
                        if "standard_code" in std:
                            ...
                            standards_text.append(
                                f"{std['standards']} - {std['description'][:100]}..."
                            )
                        ...
                        vocab.append(
                            {
                                ...
                                "definition": (
                                    description[:200]
                                    if description
                                    else f"Key concept related to {title}"
                                ),
                            }
                        )
                    ...
                    if subject == "Science":
                        vocab.extend(
                            [
                                {
                                    "term": "hypothesis",
                                    "definition": "A testable explanation for a scientific observation",
                                },
                                {
                                    "term": "variable",
                                ...
                                {
                                    "term": "control",
                                    "definition": "The standard condition in an experiment",
                                },
                            ]
                        )
    ...
    def _create_learning_path(self, topic: str, grade_level: str) -> list[dict[str, Any]]:
        """Create a structured learning path"""

        complexity = self.grade_levels.get(grade_level, {}).get("complexity", "intermediate")
    ...
    async def generate_lesson_plan(
        ...
        content = await self._generate_educational_content(subject, grade_level, topic, objectives)
...
Make it engaging and appropriate for Roblox platform."""
        ...
        try:
            ...
            for item in content_items:
                if (
                    ...
                    or query_lower in item.get("topic", "").lower()
                    or query_lower in item.get("subject", "").lower()
                ):
            ...
            if relevant_items:
                return f"Found {len(relevant_items)} relevant content items: " + "; ".join(
                    [f"{item['title']} ({item['subject']})" for item in relevant_items]
                )
            else:
    ...
    async def _analyze_student_progress(self, student_data: str) -> str:
        ...
        try:
            ...
            if student_match:
                ...
                progress_data = (
                    await self.agent_db.get_student_progress(student_id) if self.agent_db else {}
                )
                ...
                if progress_data:
                    ...
                    if progress_items:
                        completed = len(
                            [p for p in progress_items if p.get("status") == "completed"]
                        )
                        in_progress = len(
                            [p for p in progress_items if p.get("status") == "in_progress"]
                        )
                        avg_score = sum(
                            [p.get("score", 0) for p in progress_items if p.get("score")]
                        ) / len(progress_items)
    ...
    async def _execute_sparc_cycle(self, task_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "state_summary": (
                    current_state.summary if hasattr(current_state, "summary") else "state_updated"
                ),
                "policy_decision": (
                    policy_decision.action_type
                    if hasattr(policy_decision, "action_type")
                    else "enhance_quality"
                ),
                "action_success": (
                    action_result.success if hasattr(action_result, "success") else True
                ),
            ...
            return {"sparc_cycle_completed": False, "error": str(e)}
    ...
    async def _save_with_retry(
        self, content_type: str, content_data: dict[str, Any], max_retries: int = 3
    ) -> bool:
    ...
    async def update_context_with_database_data(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            objectives = (
                await self.agent_db.get_learning_objectives(subject, grade_level)
                if self.agent_db
                else []
            )
            if objectives:
            ...
            content_items = (
                await self.agent_db.get_educational_content(
            ...
            standards = (
                await self.agent_db.get_curriculum_standards(subject, grade_level)
                if self.agent_db
                else []
            )
            if standards:
