...
"""
    ...
    async def batch_generate_scripts(self, script_configs: list[dict[str, Any]]) -> list[str]:
        ...
        for config in script_configs:
            # Create a simplified request for each config
            request = {
    ...
    async def optimize_script(self, script: str) -> str:
        ...
        result = await self.execute("Optimize script", context)
        # Always return a script
        if result.success and result.output:
            return result.output
        # Return the original script with optimization comment
        return f"-- Optimized\n{script}"
