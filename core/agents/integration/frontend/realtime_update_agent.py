...
class RealtimeUpdateAgent(BaseIntegrationAgent):
    ...
    async def handle_websocket_fallback(
        self,
        ws_id: str,
        ws_connection: Any
    ) -> TaskResult:
    ...
    async def get_presence_members(
        self,
        channel_name: str
    ) -> TaskResult:
        ...
        try:
            health_status = {
                ...
                "channels": {
                    ...
                    "active": sum(1 for c in self.channels.values() if c.last_activity and
                                  (datetime.utcnow() - c.last_activity).total_seconds() < 300)
                },
                "messages": {
            ...
            if self.connection_metrics.connection_state in [ConnectionState.DISCONNECTED, ConnectionState.FAILED]:
    ...
    async def cleanup_idle_channels(
        self,
