...
# Agent imports
from .orchestrator import OrchestrationRequest, Orchestrator, WorkflowType
...
class PluginCommunicationHub:
    ...
    async def _handle_ci_cd_trigger(self, request: PluginRequest) -> PluginResponse:
        ...
        try:
            ...
            test_result = await testing_agent.execute(
                "Run CI/CD tests",
                {
                    "action": action,
                    "branch": branch,
    ...
    async def _handle_progress_update(self, request: PluginRequest) -> PluginResponse:
        ...
        try:
            ...
            await self.handle_progress_update(request)
...
async def trigger_content_generation(
    subject: str, grade_level: int, learning_objectives: list[str], **kwargs
) -> PluginResponse:
...
async def trigger_ci_cd_pipeline(
    action: str, branch: str = "main", environment: str = "development"
) -> PluginResponse:
    """Convenience function to trigger CI/CD pipeline"""

    request = PluginRequest(
        event_type=PluginEventType.CI_CD_TRIGGER,
        parameters={"action": action, "branch": branch, "environment": environment},
        priority=5,
    )
