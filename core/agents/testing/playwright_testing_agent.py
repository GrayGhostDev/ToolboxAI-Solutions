...
"""
    ...
    async def generate_test_from_user_story(
        ...
        prompt = f"""
        Generate a Playwright test for this user story:
        {user_story}

        Test Type: {test_type}
        Platform: Educational (ToolBoxAI)
...
"""
    ...
    async def analyze_coverage(
        ...
        # Check UI coverage
        ui_tests = [r for r in self.test_results if 'ui' in r.test_name.lower() or 'page' in r.test_name.lower()]
        if len(ui_tests) < 10:
        ...
        try:
            if 'execute' in task.lower():
                ...
                result = await self.generate_test_from_user_story(
                    context.get('user_story', ''),
                    context.get('test_type', 'e2e')
                )
            elif 'coverage' in task.lower():
                # Analyze coverage
                result = await self.analyze_coverage(
                    context.get('coverage_data')
                )
                ...
                result = await self.create_page_object(
                    context.get('page_name', 'Page'),
                    context.get('selectors', {})
                )
            else:
