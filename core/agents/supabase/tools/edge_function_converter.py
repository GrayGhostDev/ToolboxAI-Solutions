...
class EdgeFunctionConverterTool:
    ...
    async def convert_project(
        ...
        for i, endpoint in enumerate(endpoints):
            try:
                result = await self.convert_endpoint(
                    endpoint,
                    target_directory,
                    dry_run
                )
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Assign):
                ...
                if isinstance(node.value, ast.Call):
                    ...
                    elif (hasattr(node.value.func, 'id') and
    ...
    def _parse_route_decorator(
        ...
        if isinstance(decorator, ast.Call):
            if (isinstance(decorator.func, ast.Attribute) and
                ...
                decorator.func.value.id in app_names):
...
"""
    ...
    async def _generate_project_files(
        ...
        summary = {
            ...
            'successful_conversions': len([r for r in results.values() if r.status == ConversionStatus.SUCCESS]),
            'failed_conversions': len([r for r in results.values() if r.status == ConversionStatus.FAILED]),
            'skipped_conversions': len([r for r in results.values() if r.status == ConversionStatus.SKIPPED]),
            'results': {k: {
