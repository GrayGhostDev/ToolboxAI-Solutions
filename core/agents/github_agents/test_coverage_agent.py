...
class FileCoverage:
    ...
    def needs_attention(self) -> bool:
        ...
        return (
            self.metrics.line_coverage_percent < 80
            or self.is_critical_file
            and self.metrics.line_coverage_percent < 95
            or len(self.uncovered_lines) > 20
        )
...
class TestCoverageAgent(BaseGitHubAgent):
        ...
        try:
            ...
            await self.log_operation(
                "coverage_analysis",
                {
                    ...
                    "python_coverage": (
                        report.python_coverage.overall_coverage_percent
                        if report.python_coverage
                        else None
                    ),
                    "javascript_coverage": (
                        report.javascript_coverage.overall_coverage_percent
    ...
    async def analyze_python_coverage(self, target_directory: Path) -> Optional[CoverageMetrics]:
        ...
        """
        try:
            coverage_file = target_directory / self.config.get("test_coverage", {}).get(
                "python", {}
            ).get("coverage_file", "coverage.xml")
            ...
            logger.info(
                f"Python coverage: {metrics.line_coverage_percent:.1f}% lines, {metrics.branch_coverage_percent:.1f}% branches"
            )
            return metrics
    ...
    async def analyze_javascript_coverage(
        ...
        try:
            coverage_file = target_directory / self.config.get("test_coverage", {}).get(
                "javascript", {}
            ).get("coverage_file", "coverage/coverage-final.json")
            ...
            logger.info(
                f"JavaScript coverage: {metrics.line_coverage_percent:.1f}% lines, {metrics.branch_coverage_percent:.1f}% branches"
            )
            return metrics
    ...
    async def identify_untested_critical_paths(
        self, target_directory: Path, file_coverages: list[FileCoverage]
    ) -> list[UncoveredCodePath]:
        ...
        try:
            ...
            if report.python_coverage:
                if report.python_coverage.line_coverage_percent < thresholds.min_line_coverage:
                    ...
                    logger.warning(
                        f"Python line coverage {report.python_coverage.line_coverage_percent:.1f}% below threshold {thresholds.min_line_coverage}%"
                    )

                if report.python_coverage.branch_coverage_percent < thresholds.min_branch_coverage:
                    meets_thresholds = False
                    logger.warning(
                        f"Python branch coverage {report.python_coverage.branch_coverage_percent:.1f}% below threshold {thresholds.min_branch_coverage}%"
                    )

                if (
                    report.python_coverage.function_coverage_percent
                    < thresholds.min_function_coverage
                ):
                    meets_thresholds = False
                    logger.warning(
                        f"Python function coverage {report.python_coverage.function_coverage_percent:.1f}% below threshold {thresholds.min_function_coverage}%"
                    )
            ...
            if report.javascript_coverage:
                if report.javascript_coverage.line_coverage_percent < thresholds.min_line_coverage:
                    meets_thresholds = False
                    logger.warning(
                        f"JavaScript line coverage {report.javascript_coverage.line_coverage_percent:.1f}% below threshold {thresholds.min_line_coverage}%"
                    )

                if (
                    report.javascript_coverage.branch_coverage_percent
                    < thresholds.min_branch_coverage
                ):
                    meets_thresholds = False
                    logger.warning(
                        f"JavaScript branch coverage {report.javascript_coverage.branch_coverage_percent:.1f}% below threshold {thresholds.min_branch_coverage}%"
                    )

                if (
                    report.javascript_coverage.function_coverage_percent
                    < thresholds.min_function_coverage
                ):
                    meets_thresholds = False
                    logger.warning(
                        f"JavaScript function coverage {report.javascript_coverage.function_coverage_percent:.1f}% below threshold {thresholds.min_function_coverage}%"
                    )

            # Check critical file coverage
            for file_coverage in report.file_coverages:
                if (
                    file_coverage.is_critical_file
                    and file_coverage.metrics.line_coverage_percent
                    < thresholds.critical_file_min_coverage
                ):
                    meets_thresholds = False
                    logger.warning(
                        f"Critical file {file_coverage.file_path} coverage {file_coverage.metrics.line_coverage_percent:.1f}% below threshold {thresholds.critical_file_min_coverage}%"
                    )
    ...
    async def execute_action(self, action: str, **kwargs) -> dict[str, Any]:
        ...
        try:
            if action == "analyze":
                ...
                return {"success": True, "report": report, "format": format_type}
    ...
    async def _run_coverage_tests(self, target_directory: Path) -> dict[str, Any]:
        ...
        try:
            python_cmd = (
                self.config.get("test_coverage", {})
                .get("python", {})
                .get("coverage_command", "pytest --cov --cov-report=xml")
            )
        ...
        try:
            js_cmd = (
                self.config.get("test_coverage", {})
                .get("javascript", {})
                .get("coverage_command", "npm run test:coverage")
            )
            process = await asyncio.create_subprocess_shell(
                js_cmd,
                cwd=target_directory,
    ...
    async def _analyze_file_coverage(self, target_directory: Path) -> list[FileCoverage]:
        ...
        # Analyze JavaScript/TypeScript files
        js_files = (
            list(target_directory.rglob("*.js"))
            + list(target_directory.rglob("*.jsx"))
            + list(target_directory.rglob("*.ts"))
            + list(target_directory.rglob("*.tsx"))
        )
    ...
    async def _analyze_python_critical_paths(
        self, file_path: Path, file_coverage: FileCoverage
    ) -> list[UncoveredCodePath]:
        ...
        try:
            ...
            for match in function_pattern.finditer(content):
                ...
                if any(
                    line >= start_line and line <= start_line + 10
                    ...
                    critical_path = UncoveredCodePath(
                        ...
                        is_error_handler="error" in function_name.lower()
                        or "exception" in function_name.lower(),
    ...
    async def _analyze_javascript_critical_paths(
        self, file_path: Path, file_coverage: FileCoverage
    ) -> list[UncoveredCodePath]:
                ...
                for match in pattern.finditer(content):
                    ...
                    if any(
                        ...
                        critical_path = UncoveredCodePath(
                            ...
                            is_public_api=not function_name.startswith("_"),
                            is_error_handler="error" in function_name.lower()
                            or "catch" in function_name.lower(),
                            description=f"Uncovered function: {function_name}",
                        )
                        critical_paths.append(critical_path)
    ...
    async def _update_coverage_trends(self, report: CoverageReport) -> dict[str, list[float]]:
        ...
        try:
            ...
            # Generate trends
            trends["python"] = [
                entry["python_coverage"]
                for entry in history["entries"]
                if entry["python_coverage"] is not None
            ]
            trends["javascript"] = [
                entry["javascript_coverage"]
                for entry in history["entries"]
    ...
    async def _generate_recommendations(self, report: CoverageReport) -> list[str]:
        ...
        if report.python_coverage and report.python_coverage.overall_coverage_percent < 90:
            recommendations.append(
                f"Python coverage is {report.python_coverage.overall_coverage_percent:.1f}%. Consider adding more unit tests."
            )

        if report.javascript_coverage and report.javascript_coverage.overall_coverage_percent < 90:
            recommendations.append(
                f"JavaScript coverage is {report.javascript_coverage.overall_coverage_percent:.1f}%. Consider adding more unit tests."
            )
        ...
        if high_priority_paths:
            recommendations.append(
                f"Found {len(high_priority_paths)} high-priority uncovered critical paths. Focus on testing public APIs and error handlers."
            )
        ...
        if files_needing_attention:
            recommendations.append(
                f"{len(files_needing_attention)} files need attention. Focus on critical files first."
            )

        # Check branch coverage
        if (
            report.python_coverage
            and report.python_coverage.branch_coverage_percent
            < report.python_coverage.line_coverage_percent - 10
        ):
            recommendations.append(
        ...
        if len(recommendations) == 0:
            recommendations.append(
                "Coverage looks good! Consider adding integration tests to complement unit tests."
            )
    ...
    def _generate_markdown_report(self, report_data: dict[str, Any]) -> str:
        ...
        lines.append(
            f"**Meets Thresholds:** {'✅ Yes' if report_data['meets_thresholds'] else '❌ No'}\n"
        )
        ...
        if critical_paths:
            ...
            for path in critical_paths[:10]:  # Show top 10
                lines.append(
                    f"- **{path['function_name']}** in {path['file_path']} (Priority: {path['priority_score']})"
                )
    ...
    async def _generate_missing_tests(self, **kwargs) -> dict[str, Any]:
        ...
        try:
            kwargs.get("target_directory") or self.get_repository_root()
