...
class PoolMonitor:
    ...
    def calculate_optimal_size(self, concurrent_requests: int, avg_query_time: float) -> Dict[str, int]:
        ...
        Returns:
            Dictionary with pool_size, max_overflow, and total_connections
        """
        # Calculate base pool size based on concurrent requests and query time
        # Formula: pool_size = concurrent_requests * avg_query_time * safety_factor
        safety_factor = 1.5  # 50% safety margin
        base_pool_size = int(concurrent_requests * avg_query_time * safety_factor)

        # Ensure minimum and maximum bounds
        min_pool_size = 5
        max_pool_size = 100
        pool_size = max(min_pool_size, min(base_pool_size, max_pool_size))

        # Calculate overflow (typically 50-100% of pool size)
