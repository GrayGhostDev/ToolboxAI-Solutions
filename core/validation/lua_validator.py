...
class LuaScriptValidator:
    ...
    def _calculate_complexity(self, lua_code: str) -> float:
        ...
        decision_keywords = [
            r'\bif\b', r'\belseif\b', r'\bwhile\b', r'\bfor\b',
            r'\brepeat\b', r'\band\b', r'\bor\b'
        ]
    ...
    def generate_fix_suggestions(self, issues: List[ValidationIssue]) -> List[str]:
        ...
        for issue in issues:
            if issue.rule == "deprecated_api" and "wait()" in issue.message:
                ...
                suggestions.append("Store event connections and disconnect them when no longer needed")
            elif issue.rule == "security_risk":
                suggestions.append("Remove dangerous function calls or replace with safe alternatives")
