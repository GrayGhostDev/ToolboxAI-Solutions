...
class PerformanceMonitor:
    ...
    async def _collect_performance_snapshot(self) -> PerformanceSnapshot:
        ...
        try:
            ...
            if network_io:
                snapshot.network_io_mbps = (network_io.bytes_sent + network_io.bytes_recv) / (
                    1024 * 1024
                )
            ...
            try:
                if hasattr(monitor, "get_performance_metrics"):
                    ...
                    if component_name == "message_broker":
                        ...
                        snapshot.learning_objectives_per_minute = metrics.get(
        ...
        while self.monitoring_active:
            try:
                ...
                if degradation_alerts:
                    logger.warning(
                        f"Performance degradation detected: {'; '.join(degradation_alerts)}"
                    )
    ...
    async def _calculate_baselines(self):
        ...
        try:
            ...
            self.baselines = {
                "response_time_ms": statistics.median(
                    [s.response_time_ms for s in recent_snapshots]
                ),
                "throughput_per_minute": statistics.median(
                    [s.throughput_per_minute for s in recent_snapshots]
                ),
                "quality_score": statistics.median(
                    [s.quality_score for s in recent_snapshots if s.quality_score > 0]
                ),
                "error_rate": statistics.median([s.error_rate for s in recent_snapshots]),
                "cpu_usage_percent": statistics.median(
                    [s.cpu_usage_percent for s in recent_snapshots]
                ),
                "memory_usage_percent": statistics.median(
                    [s.memory_usage_percent for s in recent_snapshots]
                ),
            }
...
class PerformanceOptimizer:
    ...
    async def _analyze_performance_patterns(self) -> dict[str, Any]:
        ...
        try:
            ...
            for (
                metric_name,
                trend_data,
            ) in self.performance_monitor.metric_trends.items():
    ...
    async def _generate_recommendations(
        ...
        if current_performance.response_time_ms > 3000:  # > 3 seconds
            recommendations.extend(
                self._generate_latency_recommendations(current_performance, patterns)
            )

        # Throughput optimizations
        if performance_summary.get("avg_throughput", 0) < 10:  # < 10 tasks per minute
            recommendations.extend(
                self._generate_throughput_recommendations(current_performance, patterns)
            )

        # Quality optimizations
        if current_performance.quality_score < self.quality_threshold:
            recommendations.extend(
                self._generate_quality_recommendations(current_performance, patterns)
            )

        # Resource utilization optimizations
        if (
            current_performance.cpu_usage_percent > 80
            or current_performance.memory_usage_percent > 80
        ):
            recommendations.extend(
                self._generate_resource_recommendations(current_performance, patterns)
            )

        # Agent coordination optimizations
        if current_performance.coordination_efficiency < 0.7:
            recommendations.extend(
                self._generate_coordination_recommendations(current_performance, patterns)
            )

        # Educational effectiveness optimizations
        if current_performance.learning_objectives_per_minute < 2:
            recommendations.extend(
                self._generate_educational_recommendations(current_performance, patterns)
            )

        return recommendations

    def _generate_latency_recommendations(
        self, performance: PerformanceSnapshot, patterns: dict[str, Any]
    ) -> list[OptimizationRecommendation]:
    ...
    def _filter_and_prioritize_recommendations(
        ...
        if self.optimization_strategy == OptimizationStrategy.QUALITY_FIRST:
            ...
            filtered = [
                rec
                for rec in filtered
                if rec.educational_impact == "positive" or rec.quality_impact > 0
            ]
    ...
    async def apply_recommendation(self, recommendation: OptimizationRecommendation) -> bool:
        ...
        try:
            ...
            success = await self._apply_configuration_changes(
                recommendation.target_components, recommendation.configuration_changes
            )

            if success:
                logger.info(
                    f"Successfully applied optimization: {recommendation.recommendation_id}"
                )
    ...
    async def _apply_configuration_changes(
        self, target_components: list[str], configuration_changes: dict[str, Any]
    ) -> bool:
        ...
        logger.info(
            f"Applying configuration changes to {target_components}: {configuration_changes}"
        )
    ...
    async def _track_optimization_effectiveness(self, recommendation: OptimizationRecommendation):
        ...
        try:
            ...
            if current_performance:
                ...
                logger.info(
                    f"Tracked effectiveness for optimization: {recommendation.recommendation_id}"
                )
