...
class PlaywrightMCPServer:
    ...
    def _setup_handlers(self):
        ...
        async def call_tool(name: str, arguments: dict[str, Any]) -> CallToolResult:
            ...
            try:
                ...
                return CallToolResult(content=[TextContent(text=json.dumps({"error": str(e)}))])
    ...
    async def _generate_test(self, story: dict[str, Any], test_type: str = "e2e") -> dict[str, Any]:
        ...
        result = await self.generation_agent.execute_task(
            {"action": "generate_from_story", "story": story, "test_type": test_type}
        )
        ...
        return {
            ...
            "generated_code": (
                result.get("test_cases", [{}])[0].get("generated_code")
    ...
    async def _analyze_coverage(
        ...
        return {
            ...
            "branch_coverage": (
                result.get("coverage_report", {}).get("branch_coverage")
                if include_branches
                else None
            ),
    ...
    async def _heal_test(self, failure: dict[str, Any], auto_apply: bool = False) -> dict[str, Any]:
        """Auto-heal a broken test."""
        logger.info("Healing broken test")

        result = await self.healing_agent.execute_task({"action": "heal_test", "failure": failure})
    ...
    async def _visual_regression(
        self, baseline: str, current: str, threshold: float = 0.1
    ) -> dict[str, Any]:
    ...
    async def _performance_test(
        ...
        result = await self.playwright_agent.execute_task(
            {"action": "performance_test", "url": url, "metrics": metrics}
        )
    ...
    async def _accessibility_audit(self, url: str, standards: str = "wcag2aa") -> dict[str, Any]:
        """Run accessibility audit."""
        logger.info(f"Running accessibility audit for: {url} with {standards}")

        result = await self.playwright_agent.execute_task(
            {"action": "accessibility_audit", "url": url, "standards": standards}
        )
    ...
    async def _batch_test(
        self, test_suite: str, parallel_workers: int = 4, fail_fast: bool = False
    ) -> dict[str, Any]:
    ...
    async def _create_page_object(
        self, page_url: str, page_name: str, auto_detect_elements: bool = True
    ) -> dict[str, Any]:
