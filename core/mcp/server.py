...
from .auth_middleware import WebSocketAuthError, WebSocketAuthMiddleware
...
class MCPServer:
    ...
    async def register(self, websocket, path=None):
        ...
        logger.info(
            f"Client {authenticated_client.client_id} registered (user: {authenticated_client.user_id})"
        )
    ...
    async def _handle_update_context(
        ...
        source = data.get(
            "source", f"{authenticated_client.user_id}_{authenticated_client.client_id}"
        )
        ...
        logger.info(
            f"Context updated by user {authenticated_client.user_id} (client {authenticated_client.client_id})"
        )
    ...
    async def _handle_clear_context(self, data: dict, authenticated_client: AuthenticatedClient):
        ...
        if source:
            ...
            logger.info(
                f"User {user_id} cleared {len(keys_to_remove)} context entries for source {source}"
            )
        else:
            # Clear user's own context only
            user_prefix = f"{user_id}_"
            keys_to_remove = [
                k for k, v in self.context_store.items() if v.source.startswith(user_prefix)
            ]
            for key in keys_to_remove:
                del self.context_store[key]

            logger.info(
                f"User {user_id} cleared {len(keys_to_remove)} of their own context entries"
            )

        await self.broadcast_context()

    async def _handle_query_context(self, data: dict, websocket):
        ...
        await websocket.send(json.dumps({"type": "query_response", "data": filtered_context}))
    ...
    async def send_context(self, websocket):
        ...
        context_data = {key: entry.to_dict() for key, entry in self.context_store.items()}
    ...
    async def _handle_token_refresh(self, data: dict, authenticated_client: AuthenticatedClient):
        ...
        success = await self.auth_middleware.validate_token_refresh(
            authenticated_client.client_id, new_token
        )
        if not success:
        ...
        await authenticated_client.websocket.send(
            json.dumps(
                {
        ...
        logger.info(
            f"Token refreshed for user {authenticated_client.user_id} (client {authenticated_client.client_id})"
        )
...
async def main():
    ...
    server = MCPServer(port=MCPServer.DEFAULT_PORT, max_tokens=MCPServer.DEFAULT_MAX_TOKENS)
    await server.start()
