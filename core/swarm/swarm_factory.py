...
from .swarm_controller import SwarmConfig, SwarmController
from .task_distributor import DistributorConfig, TaskDistributor
from .worker_pool import WorkerConfig, WorkerPool
...
async def create_swarm_controller(
    ...
    worker_config = worker_config or WorkerConfig(
        max_workers=config.max_workers, min_workers=config.min_workers
    )
    distributor_config = distributor_config or DistributorConfig()
    consensus_config = consensus_config or ConsensusConfig(threshold=config.consensus_threshold)
    balancer_config = balancer_config or LoadBalancerConfig(strategy=config.load_balancing_strategy)
