...
class KMSEncryptionAgent(BaseAgent):
    ...
    async def _execute_recommendation(
        self, recommendation: dict[str, Any], task: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _encrypt_data(
        self, task: dict[str, Any], recommendation: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if not data:
            return {"status": "error", "message": "No data provided for encryption"}
    ...
    async def _decrypt_data(
        ...
        if not encrypted_data:
            return {"status": "error", "message": "No encrypted data provided"}
    ...
    async def _direct_kms_encrypt(
        self, data: Any, key_id: str, task: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        response = self.kms_client.encrypt(
            KeyId=key_id, Plaintext=data_bytes, EncryptionContext=encryption_context
        )
    ...
    async def _direct_kms_decrypt(
        self, encrypted_data: str, task: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "message": f"Invalid encrypted data format: {e}"}
        ...
        try:
            response = self.kms_client.decrypt(
                CiphertextBlob=ciphertext_blob, EncryptionContext=encryption_context
            )
    ...
    async def _envelope_encrypt(
        ...
        # Generate data key
        data_key_result = await self._create_data_key({"key_id": key_id, "key_spec": "AES_256"})
    ...
    async def _envelope_decrypt(
        self, envelope: dict[str, Any], task: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if not encrypted_data or not encrypted_key:
            return {"status": "error", "message": "Invalid envelope structure"}
        ...
        try:
            ...
            return {"status": "success", "decrypted_data": data}
    ...
    async def _validate_key(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            validation_result = {
                ...
                "key_usage": key_usage,
                "creation_date": (
                    key_metadata["CreationDate"].isoformat()
    ...
    async def _audit_encryption(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if audit_results["customer_managed"] > 0:
                rotation_percentage = (
                    audit_results["rotation_enabled"] / audit_results["customer_managed"]
                ) * 100
            ...
            audit_results["compliance_status"] = (
                "compliant" if not audit_results["compliance_issues"] else "non-compliant"
            )
