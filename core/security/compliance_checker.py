...
class ComplianceChecker:
    ...
    async def check_access_control(self) -> None:
        ...
        for check_id, description in checks:
            ...
            rbac_files = (
                list(self.project_root.glob("**/rbac*.py"))
    ...
    async def check_injection_protection(self) -> None:
        ...
        validation_found = (
            await self.check_code_pattern("pydantic")
            or await self.check_code_pattern("joi")
            or await self.check_code_pattern("validator")
        )
    ...
    async def check_integrity_controls(self) -> None:
        ...
        for check_id, description in integrity_checks:
            ...
            lock_files = (
                list(self.project_root.glob("**/package-lock.json"))
    ...
    async def check_logging_monitoring(self) -> None:
        ...
        logging_found = (
            await self.check_code_pattern("logging")
            or await self.check_code_pattern("logger")
            or await self.check_code_pattern("audit")
        )
    ...
    async def check_ssrf_protection(self) -> None:
        ...
        url_validation = (
            await self.check_code_pattern("validators")
            or await self.check_code_pattern("urllib.parse")
            or await self.check_code_pattern("url_validation")
        )
    ...
    async def check_rate_limiting(self) -> None:
        ...
        rate_limit_files = list(self.project_root.glob("**/rate_limit*.py")) + list(
            self.project_root.glob("**/rate_limiter*.py")
        )
    ...
    async def check_data_protection(self) -> None:
        ...
        for check_id, description in protection_features:
            ...
            if "encryption" in check_id:
                crypto_found = await self.check_code_pattern(
                    "cryptography"
                ) or await self.check_code_pattern("encrypt")
    ...
    async def check_regulatory_compliance(self) -> None:
        ...
        for standard, description in self.compliance_standards.items():
            ...
            compliance_docs = (
                list(self.project_root.glob(f"**/*{standard.lower()}*.md"))
    ...
    def generate_compliance_report(self) -> ComplianceReport:
        ...
        for category in set(c.category for c in self.checks):
            ...
            category_score = (
                (category_passed / len(category_checks) * 100) if category_checks else 0
            )
    ...
    def check_owasp_compliance_status(self) -> bool:
        ...
        critical_failures = [
            c for c in owasp_checks if c.status == "FAIL" and c.severity == "CRITICAL"
        ]
    ...
    def generate_recommendations(self) -> list[str]:
        ...
        if critical_failures:
            recommendations.append(
                f"Fix {len(critical_failures)} critical compliance failures immediately"
            )
