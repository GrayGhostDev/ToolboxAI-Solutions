...
class SecurityCoordinator:
    ...
    def generate_comprehensive_recommendations(
        self, vuln_report: dict, compliance_report: dict, rotation_report: dict
    ) -> list[str]:
    ...
    def generate_next_actions(
        ...
        if critical_issues > 0:
            next_actions.append(
                f"CRITICAL: Fix {critical_issues} critical vulnerabilities immediately"
            )

        if overall_score < self.target_security_score:
            score_gap = self.target_security_score - overall_score
            next_actions.append(
                f"URGENT: Improve security score by {score_gap:.1f}% to reach target"
            )
    ...
    async def check_security_thresholds(self, status: SecurityStatus) -> None:
        ...
        if status.overall_score < self.target_security_score:
            ...
            alerts.append(
                f"🚨 Security score below target: {status.overall_score:.1f}% (target: {self.target_security_score}%)"
            )
        ...
        if status.critical_issues > self.critical_threshold:
            alerts.append(
                f"🚨 CRITICAL: {status.critical_issues} critical vulnerabilities detected"
            )

        # Check high vulnerabilities
        if status.high_issues > self.high_threshold:
            alerts.append(
                f"⚠️  HIGH: {status.high_issues} high-severity vulnerabilities (threshold: {self.high_threshold})"
            )
    ...
    async def pre_commit_security_check(self) -> bool:
        ...
        if critical_vulns:
            logger.error(
                f"❌ Pre-commit check failed: {len(critical_vulns)} critical vulnerabilities"
            )
            return False
    ...
    async def post_fix_validation(self) -> bool:
        ...
        if status.overall_score < self.target_security_score:
            logger.error(
                f"❌ Post-fix validation failed: Security score {status.overall_score:.1f}% below target {self.target_security_score}%"
            )
            return False
        ...
        if status.critical_issues > 0:
            logger.error(
                f"❌ Post-fix validation failed: {status.critical_issues} critical issues remain"
            )
            return False
    ...
    async def deployment_security_gate(self) -> bool:
        ...
        if status.overall_score < 95.0:
            logger.error(
                f"❌ Deployment blocked: Security score {status.overall_score:.1f}% below 95%"
            )
            return False

        if status.critical_issues > 0:
            logger.error(
                f"❌ Deployment blocked: {status.critical_issues} critical vulnerabilities"
            )
            return False

        if status.high_issues > 1:
            logger.error(
                f"❌ Deployment blocked: {status.high_issues} high-severity vulnerabilities (max: 1)"
            )
            return False
        ...
        if compliance_failures:
            logger.error(
                f"❌ Deployment blocked: Compliance failures: {', '.join(compliance_failures)}"
            )
            return False
    ...
    async def save_security_alerts(self, alerts: list[str], timestamp: datetime) -> None:
        ...
        try:
            ...
            alert_data = {"timestamp": timestamp.isoformat(), "alerts": alerts}
    ...
    def log_security_status(self, status: SecurityStatus) -> None:
        ...
        logger.info(
            f"🛡️  Security Status: {status.overall_score:.1f}% (Grade: {status.security_grade})"
        )
        ...
        if status.overall_score >= self.target_security_score:
            ...
            logger.warning(
                f"⚠️  Security score below target: {status.overall_score:.1f}% < {self.target_security_score}%"
            )
        ...
        try:
            ...
            if status.critical_issues > 5:
                ...
                response_actions.append(
                    f"Emergency secret rotation: {len(rotation_results)} secrets rotated"
                )
            ...
            emergency_file = (
                self.project_root
                / f'EMERGENCY_SECURITY_REPORT_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
            )
