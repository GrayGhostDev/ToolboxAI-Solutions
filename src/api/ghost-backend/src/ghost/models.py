...
class SoftDeleteMixin:
    ...
    @declared_attr
    def deleted_at(self, cls) -> Any:
        return Column(DateTime(timezone=True), nullable=True)

    @declared_attr
    def is_deleted(self, cls) -> Any:
...
class User(Base, TimestampMixin, SoftDeleteMixin, AuditMixin):
    ...
    def full_name(self) -> str:
        """Get full name."""
        # `first_name`/`last_name` may be SQLA Column descriptors at type-check time;
        # guard with getattr to avoid Column being treated as a boolean operand.
...
class UserRepository(BaseRepository[User]):
    ...
    async def authenticate_async(self, username: str, password: str) -> Optional[User]:
        ...
        if user and user.verify_password(password):
            # Update login info
            # Assign runtime values; static analyzer may treat mapped attributes as Column[...] types
            # Use narrow ignores where pyright reports assignment issues
