...
class ConsentRecord(Base, TimestampMixin):
    ...
    # Backwards-compatible alias expected by API modules
    @property
    def parent_verified(self) -> bool:
        """Alias for `verified` when API expects a parent verification flag."""
...
class ComplianceLog(Base, TimestampMixin):
    ...
    # Compatibility: some API code expects `compliance_impact` on the log.
    @property
    def compliance_impact(self) -> Optional[str]:
        # Provide a simple mapping from `issues` or `status` to a short impact
        # string so callers can access a stable attribute without DB schema changes.
        if self.status.name == "NON_COMPLIANT":
            return "high"
