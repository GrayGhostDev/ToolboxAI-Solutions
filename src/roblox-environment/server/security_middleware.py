...
class RateLimiter:
    ...
    async def check_rate_limit(
        ...
        if self.redis_client:
            return await self._check_redis_limit(identifier, limit)
        else:
            return self._check_local_limit(identifier, limit)

    async def _check_redis_limit(self, identifier: str, limit: int) -> Tuple[bool, int]:
    ...
    def _check_local_limit(self, identifier: str, limit: int) -> Tuple[bool, int]:
...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        self.rate_limiter = RateLimiter(
            rate_limit_config or RateLimitConfig(), redis_client
        )
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if request.url.path not in self.rate_limiter.config.exclude_paths:
            # Check rate limit
