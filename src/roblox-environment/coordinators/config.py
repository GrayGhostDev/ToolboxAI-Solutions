...
class ConfigurationManager:
    ...
    def __init__(self, config_dir: Optional[str] = None):
        ...
        self.env_config_file = self.config_dir / f"coordinator_config_{os.getenv('ENVIRONMENT', 'development')}.yaml"
    ...
    def _load_env_overrides(self) -> Dict[str, Any]:
        ...
        if os.getenv('COORDINATOR_MAX_CONCURRENT_REQUESTS'):
            overrides.setdefault('main', {})['max_concurrent_requests'] = int(os.getenv('COORDINATOR_MAX_CONCURRENT_REQUESTS'))
        
        if os.getenv('COORDINATOR_ENABLE_CACHING'):
            overrides.setdefault('main', {})['enable_caching'] = os.getenv('COORDINATOR_ENABLE_CACHING').lower() == 'true'
        
        # Resource coordinator overrides
        if os.getenv('COORDINATOR_DAILY_BUDGET'):
            overrides.setdefault('resource', {})['daily_budget'] = float(os.getenv('COORDINATOR_DAILY_BUDGET'))
        
        if os.getenv('COORDINATOR_MAX_CPU_ALLOCATION'):
            overrides.setdefault('resource', {})['max_cpu_allocation'] = float(os.getenv('COORDINATOR_MAX_CPU_ALLOCATION'))
        ...
        if os.getenv('COORDINATOR_SMTP_USERNAME'):
            overrides.setdefault('error', {})['smtp_username'] = os.getenv('COORDINATOR_SMTP_USERNAME')
        
        if os.getenv('COORDINATOR_SMTP_PASSWORD'):
            overrides.setdefault('error', {})['smtp_password'] = os.getenv('COORDINATOR_SMTP_PASSWORD')
