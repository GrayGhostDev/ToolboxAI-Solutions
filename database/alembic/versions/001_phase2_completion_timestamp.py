...
def upgrade() -> None:
    ...
    op.create_index('idx_phase_completions_completion_timestamp', 'phase_completions', ['completion_timestamp'])

    # Add Phase 2 completion columns to existing users table if it exists
    try:
        op.add_column('users', sa.Column('phase2_migration_status', sa.String(20), nullable=True, default='pending'))
        op.add_column('users', sa.Column('phase2_completion_date', TIMESTAMP(timezone=True), nullable=True))
