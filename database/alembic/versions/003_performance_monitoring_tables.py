...
def upgrade() -> None:
    ...
    test_status_enum = ENUM(
        "passed", "failed", "skipped", "error", "timeout", name="test_status_type"
    )
    ...
    op.create_index(
        "idx_performance_metrics_name_service",
        "system_performance_metrics",
        ["metric_name", "service_name"],
    )
    op.create_index(
        "idx_performance_metrics_timestamp", "system_performance_metrics", ["timestamp"]
    )
