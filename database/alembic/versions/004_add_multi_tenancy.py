...
def upgrade() -> None:
    ...
    # Create enum types
    op.execute(
        "CREATE TYPE organizationstatus AS ENUM ('active', 'suspended', 'trial', 'pending', 'cancelled')"
    )
    op.execute(
        "CREATE TYPE subscriptiontier AS ENUM ('free', 'basic', 'professional', 'enterprise', 'education')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "subscription_tier",
            sa.Enum(
                "free", "basic", "professional", "enterprise", "education", name="subscriptiontier"
            ),
            default="free",
            nullable=False,
        ),
        sa.Column(
            "status",
            sa.Enum(
                "active", "suspended", "trial", "pending", "cancelled", name="organizationstatus"
            ),
            default="trial",
            nullable=False,
        ),
    ...
    op.create_index(
        "idx_organization_subscription_expires", "organizations", ["subscription_expires_at"]
    )
    ...
    op.create_index(
        "idx_org_invitation_organization", "organization_invitations", ["organization_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "log_date", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()
        ),
