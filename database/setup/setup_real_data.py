...
def create_users():
    ...
    try:
        ...
        users = [
            # Admin
            ('admin', 'admin@toolboxai.com', 'Admin123!', 'System', 'Administrator', 'Admin', 'admin', None),
            
            # Teachers
            ('john_teacher', 'john.smith@school.edu', 'Teacher123!', 'John', 'Smith', 'Mr. Smith', 'teacher', school_ids[0] if school_ids else None),
            ('jane_teacher', 'jane.doe@school.edu', 'Teacher123!', 'Jane', 'Doe', 'Ms. Doe', 'teacher', school_ids[0] if school_ids else None),
            
            # Students
            ('alex_student', 'alex.johnson@student.edu', 'Student123!', 'Alex', 'Johnson', 'Alex', 'student', school_ids[0] if school_ids else None),
            ('maria_student', 'maria.garcia@student.edu', 'Student123!', 'Maria', 'Garcia', 'Maria', 'student', school_ids[0] if school_ids else None),
            
            # Parents
            ('robert_parent', 'robert.johnson@parent.com', 'Parent123!', 'Robert', 'Johnson', 'Mr. Johnson', 'parent', None)
        ]
        
        created_users = []
        for user_data in users:
            username, email, password, first_name, last_name, display_name, role, school_id = user_data
            ...
            """, (username, email, hash_password(password), first_name, last_name, display_name, role, school_id))
        ...
        if created_users and school_ids:
            ...
            if teacher_ids:
                classes = [
                    ...
                    ('Science 201', 'Science', 10, teacher_ids[1] if len(teacher_ids) > 1 else teacher_ids[0], school_ids[0])
                ]
...
def create_educational_content():
    ...
    try:
        ...
        if teacher_ids:
            # Create courses
            courses = [
                ('Introduction to Algebra', 'Learn the fundamentals of algebraic equations and problem solving', 'Mathematics', 9, teacher_ids[0]),
                ('Basic Science Concepts', 'Explore the world of science through hands-on experiments', 'Science', 10, teacher_ids[1] if len(teacher_ids) > 1 else teacher_ids[0])
            ]
            ...
            if course_ids and class_ids:
                assignments = [
                    ('Algebra Problem Set 1', 'Solve linear equations', course_ids[0], class_ids[0] if class_ids else None, teacher_ids[0], 100),
                    ('Science Lab Report', 'Document your experiment findings', course_ids[1] if len(course_ids) > 1 else course_ids[0], class_ids[1] if len(class_ids) > 1 else class_ids[0], teacher_ids[1] if len(teacher_ids) > 1 else teacher_ids[0], 150)
                ]
