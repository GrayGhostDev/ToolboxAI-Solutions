...
def create_comprehensive_data():
    ...
    try:
        with get_session("education") as session:
            ...
            result = session.execute(text(teachers_sql), {
            ...
            # Student accounts
            students = [
                ('alice_wilson', 'alice.wilson@student.edu', 'Alice', 'Wilson', 7, 'A dedicated student who loves science experiments'),
                ('bob_davis', 'bob.davis@student.edu', 'Bob', 'Davis', 7, 'Math enthusiast and chess club member'),
                ('charlie_miller', 'charlie.miller@student.edu', 'Charlie', 'Miller', 8, 'Creative thinker with interest in history and art'),
                ('diana_garcia', 'diana.garcia@student.edu', 'Diana', 'Garcia', 7, 'Top performer in mathematics and science'),
                ('ethan_martinez', 'ethan.martinez@student.edu', 'Ethan', 'Martinez', 8, 'Aspiring game developer and coding club president'),
                ('fiona_anderson', 'fiona.anderson@student.edu', 'Fiona', 'Anderson', 7, 'Environmental science advocate'),
                ('george_taylor', 'george.taylor@student.edu', 'George', 'Taylor', 8, 'History buff and debate team captain'),
                ('hannah_thomas', 'hannah.thomas@student.edu', 'Hannah', 'Thomas', 7, 'Straight-A student with perfect attendance')
            ]
            ...
            for username, email, first, last, grade, bio in students:
                ...
                ON CONFLICT (email) DO NOTHING;
                """
                session.execute(text(student_sql), {
                    "username": username, "email": email, "password_hash": hash_password("Student123!"),
                    "first_name": first, "last_name": last, "grade_level": grade, "bio": bio
                })
            ...
            science_content = [
                {
                    ...
                    'subject': 'Science',
                    'grade': 7,
                    'objectives': ['Identify planets in our solar system', 'Understand orbital mechanics', 'Learn about planetary characteristics'],
                    'description': 'Comprehensive exploration of our solar system, including all planets, their moons, and the sun.',
                    'lessons': [
                        ('Introduction to the Solar System', 'Overview of our cosmic neighborhood', 45),
                ...
                },
                {
                    ...
                    'subject': 'Science',
                    'grade': 7,
                    'objectives': ['Understand cell structure', 'Learn about organelles', 'Compare plant and animal cells'],
            ...
            math_content = [
                {
                    ...
                    'subject': 'Mathematics',
                    'grade': 7,
                    'objectives': ['Solve one-step equations', 'Solve two-step equations', 'Graph linear equations'],
                ...
                },
                {
                    ...
                    'subject': 'Mathematics',  
                    'grade': 8,
                    'objectives': ['Understand angles and shapes', 'Calculate perimeter and area', 'Learn geometric proofs'],
            ...
            history_content = [
                {
                    ...
                    'subject': 'History',
                    'grade': 7,
                    'objectives': ['Study major ancient civilizations', 'Understand cultural development', 'Learn about historical impact'],
            ...
            for content in all_content:
                ...
                for obj in content['objectives']:
                    ...
                    RETURNING id;
                    """
                    result = session.execute(text(obj_sql), {
                        "title": obj[:100], "description": obj, "subject": content['subject'], "grade_level": content['grade']
                    })
                ...
                result = session.execute(text(content_sql), {
                    "title": content['title'],
                ...
                if content_id and 'lessons' in content:
                    for idx, (lesson_title, lesson_desc, duration) in enumerate(content['lessons'], 1):
                        lesson_sql = """
                        INSERT INTO lessons (
                        ...
                        """
                        session.execute(text(lesson_sql), {
            ...
            quiz_data = [
                {
                    ...
                    'questions': [
                        ('What is the largest planet in our solar system?', 'multiple_choice',
                         ['Jupiter', 'Saturn', 'Earth', 'Mars'], 0, 10),
                        ('How many planets are in our solar system?', 'multiple_choice',
                         ['7', '8', '9', '10'], 1, 10),
                        ('Which planet is known as the Red Planet?', 'multiple_choice',
                         ['Venus', 'Mars', 'Jupiter', 'Mercury'], 1, 10),
                        ('What is the closest planet to the Sun?', 'multiple_choice',
                         ['Venus', 'Earth', 'Mercury', 'Mars'], 2, 10),
                        ('Which planet has the most moons?', 'multiple_choice',
                         ['Earth', 'Mars', 'Jupiter', 'Saturn'], 3, 15),
                        ('True or False: The Earth is the third planet from the Sun.', 'true_false',
                         ['True', 'False'], 0, 5),
                        ('True or False: Pluto is classified as a planet.', 'true_false',
                         ['True', 'False'], 1, 5),
                        ('Name the four inner planets in order from the Sun.', 'short_answer',
                         ['Mercury, Venus, Earth, Mars'], 0, 20),
                        ('What keeps planets in orbit around the Sun?', 'short_answer',
                         ['Gravity'], 0, 15),
                        ('How long does it take Earth to orbit the Sun?', 'multiple_choice',
                         ['365 days', '30 days', '24 hours', '12 months'], 0, 10)
                    ]
                },
                {
                    ...
                    'questions': [
                        ('Solve for x: 2x + 5 = 13', 'multiple_choice',
                         ['x = 4', 'x = 6', 'x = 8', 'x = 9'], 0, 15),
                        ('Solve for y: y - 7 = 15', 'multiple_choice',
                         ['y = 8', 'y = 22', 'y = 15', 'y = 7'], 1, 10),
                        ('What is the slope of y = 3x + 2?', 'multiple_choice',
                         ['2', '3', '-3', '1/3'], 1, 10),
                        ('True or False: x = 5 is a solution to 2x - 10 = 0', 'true_false',
                         ['True', 'False'], 0, 10),
                        ('If 3x = 18, what is x?', 'short_answer',
                         ['6'], 0, 10),
                        ('Solve: 4x + 2 = 2x + 10', 'short_answer',
                         ['x = 4', '4'], 0, 20),
                        ('What is the y-intercept of y = 2x - 5?', 'multiple_choice',
                         ['-5', '2', '5', '-2'], 0, 15),
                        ('Which equation represents a horizontal line?', 'multiple_choice',
                         ['y = 5', 'x = 5', 'y = x', 'y = 2x'], 0, 15),
                        ('True or False: All linear equations have exactly one solution', 'true_false',
                         ['True', 'False'], 1, 10),
                        ('Solve for x: 5(x - 2) = 15', 'multiple_choice',
                         ['x = 1', 'x = 3', 'x = 5', 'x = 7'], 2, 15)
                    ]
                }
            ]
            ...
            for quiz in quiz_data:
                ...
                result = session.execute(text(quiz_sql), {
                    "title": quiz['title'],
                ...
                # Create questions
                if quiz_id:
                    for idx, (question, q_type, options, correct_idx, points) in enumerate(quiz['questions'], 1):
                        question_sql = """
                        ...
                        correct_answer = options[correct_idx] if isinstance(options[correct_idx], str) else str(options[correct_idx])
                        session.execute(text(question_sql), {
                            "quiz_id": quiz_id,
            ...
            for student_id, student_name in student_data:
                for content_id, content_title in content_data:
                    ...
                    session.execute(text(progress_sql), {
                        "user_id": student_id,
                        "content_id": content_id,
