...
async def verify_database_setup():
    ...
    database_url = os.getenv(
        "DATABASE_URL",
        "postgresql+asyncpg://eduplatform:eduplatform2024@localhost:5432/educational_platform_dev",
    )
        ...
        try:
            ...
            for enum_name in expected_enums:
                ...
                if count > 0:
                    result = await session.execute(
                        text(
                            f"SELECT enumlabel FROM pg_enum WHERE enumtypid = (SELECT oid FROM pg_type WHERE typname = '{enum_name}') ORDER BY enumsortorder"
                        )
                    )
            ...
            for table_name, index_name in key_indexes:
                result = await session.execute(
                    text(
                        f"SELECT COUNT(*) FROM pg_indexes WHERE tablename = '{table_name}' AND indexname = '{index_name}'"
                    )
                )
                count = result.scalar()
                status = "✅" if count > 0 else "❌"
            ...
            fk_queries = [
                (
                    "roblox_sessions -> lessons",
                    "SELECT COUNT(*) FROM information_schema.table_constraints WHERE constraint_name = 'roblox_sessions_lesson_id_fkey'",
                ),
                (
                    "roblox_sessions -> users",
                    "SELECT COUNT(*) FROM information_schema.table_constraints WHERE constraint_name = 'roblox_sessions_teacher_id_fkey'",
                ),
                (
                    "roblox_content -> lessons",
                    "SELECT COUNT(*) FROM information_schema.table_constraints WHERE constraint_name = 'roblox_content_lesson_id_fkey'",
                ),
                (
                    "roblox_content -> roblox_templates",
                    "SELECT COUNT(*) FROM information_schema.table_constraints WHERE constraint_name = 'roblox_content_template_id_fkey'",
                ),
                (
                    "roblox_player_progress -> roblox_sessions",
                    "SELECT COUNT(*) FROM information_schema.table_constraints WHERE constraint_name = 'roblox_player_progress_session_id_fkey'",
                ),
            ]
            ...
            unique_constraints = [
                (
                    ...
                    "roblox_sessions_websocket_session_id_key",
                ),
                (
                    "roblox_player_progress",
                    "session_id, student_id, roblox_user_id",
                    "roblox_player_progress_session_id_student_id_roblox_user_id_key",
                ),
            ]
            ...
            for table_name, columns, constraint_name in unique_constraints:
                result = await session.execute(
                    text(
                        f"SELECT COUNT(*) FROM information_schema.table_constraints WHERE constraint_name = '{constraint_name}' AND constraint_type = 'UNIQUE'"
                    )
                )
                count = result.scalar()
                status = "✅" if count > 0 else "❌"
            ...
            for table_name, desc in check_constraints:
                result = await session.execute(
                    text(
                        f"SELECT COUNT(*) FROM information_schema.check_constraints WHERE constraint_name LIKE '{table_name}_%'"
                    )
                )
                count = result.scalar()
                status = "✅" if count > 0 else "❌"
            ...
            for desc, query in performance_queries:
                try:
                    ...
                    if plan:
                        ...
                        print(
                            f"   {status} {desc} - {'Uses indexes' if uses_index else 'May need optimization'}"
                        )
                except Exception as e:
            ...
            stats_queries = [
                (
                    "Total Roblox tables",
                    "SELECT COUNT(*) FROM information_schema.tables WHERE table_name LIKE 'roblox_%'",
                ),
                (
                    "Total indexes on Roblox tables",
                    "SELECT COUNT(*) FROM pg_indexes WHERE tablename LIKE 'roblox_%'",
                ),
                (
                    "Total constraints on Roblox tables",
                    "SELECT COUNT(*) FROM information_schema.table_constraints WHERE table_name LIKE 'roblox_%'",
                ),
            ]
