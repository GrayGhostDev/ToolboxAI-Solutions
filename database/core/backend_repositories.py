...
class BaseRepository:
    ...
    async def get_by_id(
        ...
        if relationships:
            query = self.optimizer.optimize_relationships(
                query, self.model, relationships
            )
        
        if columns:
            query = self.optimizer.select_columns(
                query, self.model, columns
            )
    ...
    async def get_many(
        ...
        Returns:
            List of entities
        """
        return await self.data_loader.load_many(
            self.model, ids, relationships
        )
...
class UserRepository(BaseRepository):
    ...
    async def get_user_with_enrollments(self, user_id: UUID) -> Optional[User]:
...
class CourseRepository(BaseRepository):
    ...
    async def get_course_with_lessons(self, course_id: UUID) -> Optional[Course]:
    ...
    async def get_courses_with_enrollment_count(
        ...
        query = (
            select(
                Course,
                func.coalesce(enrollment_count_subquery.c.enrollment_count, 0).label("enrollment_count")
            )
            .outerjoin(
                enrollment_count_subquery,
                Course.id == enrollment_count_subquery.c.course_id
            )
            .limit(limit)
            .offset(offset)
        )
...
class LessonRepository(BaseRepository):
    ...
    async def get_lessons_with_content(
        self,
        course_id: UUID
    ) -> List[Lesson]:
...
class ProgressRepository(BaseRepository):
    ...
    async def get_user_course_progress(
        ...
        completed_lessons = sum(
            1 for p in progress_records if p.completed
        )
        
        total_progress = sum(
            p.progress_percentage for p in progress_records
        )
        ...
        return {
            ...
            "overall_progress": (
                total_progress / total_lessons if total_lessons > 0 else 0
            ),
            "lessons": [
                {
                    ...
                    "progress": (
                        ...
                        if lesson.id in progress_by_lesson else 0
                    ),
                    "completed": (
                        progress_by_lesson[lesson.id].completed
                        if lesson.id in progress_by_lesson else False
                    )
                }
