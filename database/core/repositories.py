...
class BaseRepository(ABC, Generic[ModelType]):
    ...
    async def update(self, id: uuid.UUID, **data) -> ModelType | None:
...
class UserRepository(BaseRepository[User]):
    ...
    async def create_user(
        self, email: str, username: str, password: str, role: UserRole = UserRole.STUDENT, **kwargs
    ) -> User:
        ...
        user = await self.create(
            email=email, username=username, password_hash=password_hash, role=role, **kwargs
        )
    ...
    async def get_by_roblox_id(self, roblox_user_id: str) -> User | None:
        """Get user by Roblox user ID"""
        stmt = select(User).where(User.roblox_user_id == roblox_user_id)
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()

    async def get_with_progress(self, user_id: uuid.UUID) -> User | None:
        """Get user with progress records loaded"""
        stmt = (
            select(User)
            .options(selectinload(User.progress_records), selectinload(User.achievements))
...
class CourseRepository(BaseRepository[Course]):
    ...
    async def get_with_lessons(self, course_id: uuid.UUID) -> Course | None:
    ...
    async def get_by_subject_grade(self, subject: str, grade_level: int) -> list[Course]:
    ...
    async def get_user_courses(self, user_id: uuid.UUID) -> list[Course]:
        ...
        stmt = select(Course).join(Enrollment).where(Enrollment.user_id == user_id)
        result = await self.session.execute(stmt)
        return result.scalars().all()
...
class LessonRepository(BaseRepository[Lesson]):
    ...
    async def get_with_content(self, lesson_id: uuid.UUID) -> Lesson | None:
        """Get lesson with all content loaded"""
        stmt = (
            select(Lesson)
            .options(selectinload(Lesson.content_items), selectinload(Lesson.quizzes))
    ...
    async def get_by_course(
        self, course_id: uuid.UUID, only_published: bool = True
    ) -> list[Lesson]:
    ...
    async def get_next_lesson(self, course_id: uuid.UUID, current_order: int) -> Lesson | None:
...
class QuizRepository(BaseRepository[Quiz]):
    ...
    async def get_by_lesson(self, lesson_id: uuid.UUID) -> list[Quiz]:
        ...
        stmt = select(Quiz).where(and_(Quiz.lesson_id == lesson_id, Quiz.is_active == True))
        result = await self.session.execute(stmt)
        return result.scalars().all()

    async def create_quiz_attempt(self, quiz_id: uuid.UUID, user_id: uuid.UUID) -> QuizAttempt:
    ...
    async def submit_quiz_response(
        self, attempt_id: uuid.UUID, question_id: uuid.UUID, user_answer: Any
    ) -> QuizResponse:
        ...
        stmt = select(QuizResponse).where(
            and_(QuizResponse.attempt_id == attempt_id, QuizResponse.question_id == question_id)
        )
        result = await self.session.execute(stmt)
    ...
    async def complete_quiz_attempt(self, attempt_id: uuid.UUID) -> QuizAttempt | None:
...
class ProgressRepository(BaseRepository[UserProgress]):
    ...
    async def get_or_create(self, user_id: uuid.UUID, lesson_id: uuid.UUID) -> UserProgress:
    ...
    async def get_course_progress(self, user_id: uuid.UUID, course_id: uuid.UUID) -> dict[str, Any]:
...
class AnalyticsRepository(BaseRepository[Analytics]):
    ...
    async def track_event(
        ...
        **kwargs,
        ...
        event = Analytics(
            user_id=user_id, event_type=event_type, event_data=event_data or {}, **kwargs
        )
    ...
    async def get_user_analytics(
        self, user_id: uuid.UUID, event_type: str | None = None, limit: int = 100
    ) -> list[Analytics]:
...
class AchievementRepository(BaseRepository[Achievement]):
    ...
    async def check_user_achievements(
        self, user_id: uuid.UUID, user_stats: dict[str, Any]
    ) -> list[Achievement]:
        ...
        stmt = select(UserAchievement.achievement_id).where(
            and_(UserAchievement.user_id == user_id, UserAchievement.completed == True)
        )
        result = await self.session.execute(stmt)
    ...
    def _check_requirements(self, requirements: dict[str, Any], user_stats: dict[str, Any]) -> bool:
...
class LeaderboardRepository(BaseRepository[Leaderboard]):
    ...
    async def update_user_score(
        self, user_id: uuid.UUID, points: int, update_streak: bool = True
    ) -> Leaderboard | None:
    ...
    async def get_top_users(self, limit: int = 10, timeframe: str = "all") -> list[dict[str, Any]]:
        ...
        stmt = (
            select(Leaderboard, User.username, User.display_name, User.avatar_url)
