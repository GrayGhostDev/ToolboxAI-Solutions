...
class PerformanceValidator:
    ...
    async def measure_query_performance(
        ...
        if not times:
            return {"query_name": query_name, "status": "FAILED", "error": "All iterations failed"}
    ...
    async def run_dashboard_queries(self, conn: asyncpg.Connection):
        ...
        for query_test in test_queries:
            ...
            result = await self.measure_query_performance(
                conn, query_test["name"], query_test["query"], query_test.get("params", ())
            )
    ...
    async def run_system_queries(self, conn: asyncpg.Connection):
        ...
        for query_test in system_queries:
            result = await self.measure_query_performance(
                conn, query_test["name"], query_test["query"]
            )
            self.results.append(result)
    ...
    async def run_all_tests(self) -> dict[str, Any]:
        ...
        print(
            f"Overall Average: {summary['overall_avg_time_ms']}ms (target: <{summary['target_time_ms']}ms)"
        )
