...
class RobloxDatabaseHelper:
    ...
    async def store_generated_content(
        session, lesson_id: str, content_type: str, content_data: dict[str, Any], generated_by: str
    ) -> RobloxContent:
    ...
    async def get_content_for_lesson(
        session, lesson_id: str, content_type: str | None = None
    ) -> list[RobloxContent]:
    ...
    async def create_session(
        session, lesson_id: str, teacher_id: str, place_id: int, **kwargs
    ) -> RobloxSession:
    ...
    async def track_student_progress(
        session, student_id: str, lesson_id: str, progress_data: dict[str, Any]
    ):
        ...
        query = select(StudentProgress).where(
            StudentProgress.student_id == student_id, StudentProgress.lesson_id == lesson_id
        )
        result = await session.execute(query)
        ...
        if progress:
            ...
            if "milestones" in progress_data:
                ...
                progress.milestones_completed = list(set(existing_milestones + new_milestones))
        else:
