...
class TenantConfig:
    ...
    FEATURE_FLAGS = {
        ...
        "enable_subscription_limits": os.getenv("ENABLE_SUBSCRIPTION_LIMITS", "true").lower()
        == "true",
        "enable_tenant_isolation_strict": os.getenv(
            "ENABLE_TENANT_ISOLATION_STRICT", "true"
        ).lower()
        == "true",
    }
...
class SecurityConfig:
    ...
    # Access control
    SUPER_ADMIN_EMAILS = (
        os.getenv("SUPER_ADMIN_EMAILS", "").split(",") if os.getenv("SUPER_ADMIN_EMAILS") else []
    )
    ENABLE_IP_WHITELIST = os.getenv("ENABLE_IP_WHITELIST", "false").lower() == "true"
    ALLOWED_IP_RANGES = (
        os.getenv("ALLOWED_IP_RANGES", "").split(",") if os.getenv("ALLOWED_IP_RANGES") else []
    )
