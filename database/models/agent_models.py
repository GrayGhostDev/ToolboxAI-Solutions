...
class AgentInstance(TenantBaseModel):
    ...
    agent_type = Column(
        Enum(AgentType, values_callable=lambda x: [e.value for e in x]), nullable=False, index=True
    )
    status = Column(
...
class AgentExecution(TenantBaseModel):
    ...
    agent_instance_id = Column(
        UUID(as_uuid=True), ForeignKey("agent_instances.id"), nullable=False, index=True
    )
    agent_type = Column(
        Enum(AgentType, values_callable=lambda x: [e.value for e in x]), nullable=False, index=True
    )
...
class AgentConfiguration(TenantBaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "version",
            "agent_type",
            name="uq_org_config_name_version_type",
        ),
