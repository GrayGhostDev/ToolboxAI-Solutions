...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_class_subject_grade", "classes", ["subject", "grade_level"], schema="toolboxai"
    )
    ...
    op.create_index(
        "idx_enrollment_student", "class_enrollments", ["student_id"], schema="toolboxai"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "DROP TRIGGER IF EXISTS update_class_enrollments_updated_at ON toolboxai.class_enrollments"
    )
