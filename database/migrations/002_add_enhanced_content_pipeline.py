...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('created_at', sa.DateTime(), nullable=False, default=sa.func.now()),
        sa.PrimaryKeyConstraint('id'),
        sa.ForeignKeyConstraint(['content_id'], ['enhanced_content_generations.id'], ondelete='CASCADE'),
    )

    # Create indexes for content_quality_metrics
    op.create_index('idx_cqm_content_score', 'content_quality_metrics', ['content_id', 'overall_quality_score'])
