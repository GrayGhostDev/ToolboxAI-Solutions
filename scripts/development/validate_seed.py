        ...
        async with db_manager.get_connection() as conn:
            ...
            for course in courses_without_lessons:
                self.warnings.append(
                    f"Course '{course['title']}' ({course['code']}) has no lessons"
                )
            ...
            for course in invalid_grades:
                issues.append(
                    f"Invalid grade level for course '{course['title']}': {course['grade_level']}"
                )
    ...
    async def validate_lessons(self) -> tuple[bool, list[str]]:
        ...
        async with db_manager.get_connection() as conn:
            ...
            for dup in duplicate_orders:
                issues.append(
                    f"Duplicate order_index {dup['order_index']} in course {dup['course_id']}"
                )
    ...
    async def validate_content(self) -> tuple[bool, list[str]]:
        ...
        async with db_manager.get_connection() as conn:
            ...
            for content in invalid_status:
                issues.append(
                    f"Invalid status for content '{content['title']}': {content['status']}"
                )
    ...
    async def validate_quizzes(self) -> tuple[bool, list[str]]:
        ...
        async with db_manager.get_connection() as conn:
            ...
            for quiz in invalid_scores:
                issues.append(
                    f"Invalid passing score for quiz '{quiz['title']}': {quiz['passing_score']}"
                )
    ...
    async def validate_relationships(self) -> tuple[bool, list[str]]:
        ...
        async with db_manager.get_connection() as conn:
            ...
            if invalid_enrollments:
                issues.append(
                    f"Found {len(invalid_enrollments)} enrollments with invalid references"
                )
            ...
            if invalid_progress:
                issues.append(
                    f"Found {len(invalid_progress)} progress records with invalid user references"
                )
            ...
            if invalid_attempts:
                issues.append(
                    f"Found {len(invalid_attempts)} quiz attempts with invalid references"
                )
    ...
    async def generate_report(self) -> dict[str, Any]:
        ...
        report["summary"] = {
            ...
            "populated_tables": sum(
                1 for count in self.stats.get("table_counts", {}).values() if count > 0
            ),
...
async def main():
    ...
    try:
        ...
        report_path = (
            ...
            / f"seed_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
