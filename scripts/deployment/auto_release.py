...
class AutoReleaser:
    ...
    def analyze_commits(self, since_tag: str | None = None) -> dict:
    ...
    def bump_version(
        self, current: semantic_version.Version, bump_type: str
    ...
    def create_git_tag(self, version: str, message: str):
        ...
        try:
            ...
            # Push tag to remote
            subprocess.run(
                ["git", "push", "origin", tag_name], check=False  # Don't fail if can't push
            )
    ...
    async def deploy_to_staging(self, version: str):
        ...
        if deploy_script.exists():
            result = subprocess.run(
                [str(deploy_script), "staging", version], capture_output=True, text=True
            )
...
async def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force release even without terminal approval"
    )
