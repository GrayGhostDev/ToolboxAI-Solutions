...
class CICDMetrics:
    ...
    def record_incident(
        self, severity: str, resolution_time: int, root_cause: str, affected_service: str
    ):
        """Record an incident."""
        conn = sqlite3.connect(self.metrics_db)
        cursor = conn.cursor()
    ...
    async def collect_all_metrics(self) -> dict:
        ...
        if self.redis_available:
            self.redis_client.publish("terminal:debugger:ci_metrics", json.dumps(all_metrics))
    ...
    def generate_metrics_report(self, output_file: str | None = None):
        ...
        report.append("-" * 40)
        report.append(
            f"Deployment Frequency: {metrics['dora']['deployment_frequency']} deploys/day"
        )
        ...
        if metrics["github"].get("workflow_runs"):
            ...
            success_count = sum(
                1 for run in metrics["github"]["workflow_runs"] if run["conclusion"] == "success"
            )
            total_count = len(metrics["github"]["workflow_runs"])

            report.append(
                f"Success Rate: {(success_count/total_count*100):.1f}% ({success_count}/{total_count})"
            )
            report.append("")
    ...
    def generate_recommendations(self, metrics: dict) -> list[str]:
        ...
        if dora["deployment_frequency"] < 0.25:
            recommendations.append(
                "Increase deployment frequency - aim for at least weekly deployments"
            )
        ...
        if dora["mean_time_to_recovery"] > 60:
            recommendations.append(
                "Improve incident response - implement better monitoring and rollback procedures"
            )

        # Change failure rate
        if dora["change_failure_rate"] > 15:
            recommendations.append(
                "Reduce failure rate - improve testing and code review practices"
            )

        # Build success rate
        if metrics["additional"]["build_success_rate"] < 90:
            recommendations.append(
                "Improve build stability - fix flaky tests and environment issues"
            )
        ...
        if not recommendations:
            recommendations.append(
                "Great job! Continue monitoring and maintaining current performance levels"
            )
...
async def main():
    ...
    parser.add_argument("--charts", action="store_true", help="Generate trend charts")
    parser.add_argument(
        "--record-deployment",
        nargs=4,
