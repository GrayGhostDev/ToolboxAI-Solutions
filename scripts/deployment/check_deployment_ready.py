...
class DeploymentReadinessChecker:
    def __init__(self):
        ...
        self.services = {
            ...
            "mcp": {"url": "ws://127.0.0.1:9876", "name": "MCP WebSocket", "required": False},
            "ghost": {
    ...
    def check_database(self) -> bool:
        ...
        try:
            ...
            conn = psycopg2.connect(os.getenv("DATABASE_URL", "postgresql://localhost/toolboxai"))
            cursor = conn.cursor()
    ...
    async def run_readiness_check(self) -> bool:
        ...
        for service_id, status in service_status.items():
            ...
            print(
                f"{icon} {status['name']}: {'Healthy' if status['healthy'] else status['message']}"
            )
        ...
        print(
            f"{'✅' if db_ready else '❌'} PostgreSQL: {'Connected' if db_ready else 'Not available'}"
        )
