...
# Check disk usage for the filesystem containing the project root
CURRENT_USAGE=$(df -h "$PROJECT_ROOT" | awk 'NR==2 {print int($5)}')
...
if [ $CURRENT_USAGE -gt $CRITICAL_THRESHOLD ]; then
    ...
    # Check new usage
    NEW_USAGE=$(df -h "$PROJECT_ROOT" | awk 'NR==2 {print int($5)}')
